...
class TimeDecayCompaction:
    ...
    def compute(self, grid):
        ...
        ds_dt = xr.DataArray(
            ...
            dims=["time", "timestep"],
            coords=dict(
                time=("time", grid.time.values[1:]),
                timestep=("timestep", grid.time.values[1:]),
            ),
        )

        time_decay = xr.apply_ufunc(_utils.get_time_decay, ds_dt, grid.tau)

        strain = xr.apply_ufunc(
            time_decay_strain,
            grid.dx,
            grid.dy,
            grid.thickness,
...
class RateTypeCompaction:
    ...
    def compute_1D(self, grid):
        P = grid.pressures
        sigma_vert_Pascal = (
            grid.density.astype(float) * 9.81 * grid.depths.astype(float)
        )  # in Pascal
        ...
        if (sigma < 0).any():
            raise (
                Exception(
                    f"Pressure ({round(np.max(P.values), 3)}) higher than vertical pressure ({round(np.max(sigma_vert_bar.values),3)})"
                )
            )
    ...
    def compute_grid(self, grid):
        ...
        sigma, _, _, _, _ = xr.broadcast(
            sigma, grid.y, grid.x, grid.reservoir, grid.time
        )
        compaction_coefficients, _, _ = xr.broadcast(
            grid.compaction_coefficients, grid.x, grid.y
        )
        cmref, _, _ = xr.broadcast(
            xr.where(grid.cmref <= 0, 1, grid.cmref), grid.x, grid.y
        )
        b, _, _ = xr.broadcast(xr.where(grid.b <= 0, 1, grid.b), grid.x, grid.y)
        refference_stress_rates, _, _ = xr.broadcast(
            xr.where(grid.reference_stress_rates <= 0, 1, grid.reference_stress_rates),
            grid.x,
            grid.y,
        )

        start_inner_itterations = 1000
...
def linear_strain(
    dx, dy, thickness, compaction_coefficients, initial_pressures, pressures
):
    return (
        dx * dy * thickness * compaction_coefficients * (initial_pressures - pressures)
    )
...
def RTiCM(
    timesteps, sigma, cmd, cmref, b, reference_stress_rates, number_of_inner_iterations
):
    ...
    for i in range(0, number_of_timesteps - 1):
        ...
        for j in range(0, number_of_inner_iterations):
            _strain = (
                shear_strain + direct_strain
            )  # _strain is current strain, with inner iterations

            cm = (_strain - strain0) / (sigma[i] + (j - 1) * sig_inner)
            shear_strain_rate = (
                reference_stress_rates * (cm - cmd) * (cm / cmref) ** (-1 / b)
            )
            shear_strain = (
                shear_strain
                + shear_strain_rate * dt[i + 1] / number_of_inner_iterations
            )
...
@njit(parallel=True)
def RTiCM_reservoirs(
    timesteps, sigma, cmd, cmref, b, reference_stress_rates, number_of_inner_itterations
):
    ...
    for i in range(len(strain)):
        strain[i] = RTiCM(
            ...
            cmref[i],
            b[i],
            reference_stress_rates[i],
            number_of_inner_itterations,
        )
    return strain


# @njit(parallel = True) # Causes errors
def RTiCM2D_reservoirs(
    timesteps, sigma, cmd, cmref, b, reference_stress_rates, number_of_inner_itterations
):
    strain = np.zeros(sigma.shape)
...
def RTiCM2D(
    ...
    for i in range(0, number_of_timesteps - 1):
        ...
        for j in range(0, number_of_inner_itterations):
            ...
            shear_strain = (
                shear_strain
                + shear_strain_rate * dt[i + 1] / number_of_inner_itterations
            )
            direct_strain = cmd * (sigma[:, :, i] + j * sig_inner - sigma_ref)
