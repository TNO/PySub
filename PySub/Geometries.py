...
class GeometryPoint(_Points.Point):
    ...
    def mask(self, grid):
        ...
        """
        ix, iy = np.searchsorted(grid.x, self.x), np.searchsorted(
            grid.y, self.y
        )
    ...
    def in_bound(self, *args):
        ...
        if bounds is not None:
            ...
            if not _utils.is_point_in(
                bounds_as_polygon, self.coordinates
            ).all():
...
class GeometryRaster:
    ...
    def mask(self, grid):
        ...
        xr_grid = xr.Dataset(
            coords={"x": np.unique(self.X), "y": np.unique(self.Y)}
        )
    ...
    def in_bound(self, bound, dx):
        if bound is not None:
            ...
            if not _utils.is_point_in(
                bounds_as_polygon, self.coordinates, radius=dx
            ).all():
...
class GeometryPolygon:
    ...
    def mask(self, grid):
        ...
        for shape in self.shapes:
            exteriors, interiors = _shape_utils.extract_poly_coords(shape)
            for exterior in exteriors:
                is_in_exteriors = _grid_utils.get_mask_from_shp(
                    exterior, grid.x, grid.y
                )
                mask = mask | is_in_exteriors
            for interior in interiors:
                is_in_interior = _grid_utils.get_mask_from_shp(
                    interior, grid.x, grid.y
                )
...
def from_representative_values(
    ...
    if geometry_type == "point":
        ...
        geometry = GeometryPolygon(
            representative_parameters[geometry_type], kwargs=shape_kwargs
        )
    else:
...
def fetch(files, scatter_kwargs={}, shape_kwargs={}, raster_kwargs={}):
        ...
        if isinstance(f, str):
            if f.endswith(".shp"):
                ...
                try:
                    ...
                    try:
                        df = pd.read_csv(
                            f, delimiter=",", header=0, decimal="."
                        )
                        x = df[df.columns[0]].values.astype(float)
                    except:
                        raise Exception(
                            'Invalid delimiter encountered in {f}, use ";" or ",".'
                        )
                if (
                    len(df.columns) == 3
                    and len(df[df.columns[2]].unique()) != 1
                ):
                    values, X, Y = _shape_utils.load_raster_from_csv(f)
                    values = values.reshape(values.shape[:-1])

                    geometries.append(
                        GeometryRaster(X, Y, values, raster_kwargs)
                    )
                    ...
                    raise Exception(
                        f"Input csv {f} has invalid number of rows: {len(df.columns)}. Use 3 (x, y, mask) for a raster or 2 (x, y) for a polygon."
                    )
            else:
