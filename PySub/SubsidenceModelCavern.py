...
class SubsidenceModel(_SubsidenceModelBase.SubsidenceModel):
    ...
    def vars_to_calculate(self):
        return ['subsidence_model_type', 'depth_to_basements', 'depths', 'lengths', 'knothe_angles', 'volume_change', 'viscous_tau', 'depth_to_basements_moved']
    ...
    def idxy(self):
        if self.hasattr('shapes') and self.built:
            ...
            if not self.hasattr('shapes'):
                raise Exception('No coordinates set. Set coordinates before masking the grid.')
            if not self.built:
                raise Exception('Build grid before masking the grid for caverns.')
    ...
    def move_basements(self):
        ...
        """
        if (self.hasattr('depth_to_basements') and 
            self.hasattr('viscous_tau') and 
            self.hasattr('depth_to_basements_moved')
            ): # Has all the ingredients to calculate moving basement
            time = xr.apply_ufunc(
                _utils.datetime_to_years_as_float,
                self.grid.time
                ) * 365.25*24*3600
            
            moving_rigid_basement = xr.apply_ufunc(_utils.moving_rigid_basement,
    ...
    def seperate_depths(self):
        ...
        if self.hasattr('lengths') and self.hasattr('depths'):
            ...
            check_for_layer_thickness['layer'] = np.arange(number_of_layers.max().values) + 1
            check_for_layer_thickness['thickness'] = (
                self.lengths - self.dx * (check_for_layer_thickness['layer'] - 1)
                )
            
            layer_thickness = check_for_layer_thickness['thickness'].clip(0, self.dx)
    ...
    def set_lengths(self, lengths):
        if not _utils.is_iterable(lengths):
            raise Exception('Lengths set must be iterable objects with the dimensions (m), where m is the number of caverns.')
    ...
    def set_depth_to_basements_moved(self, depth_to_basements_moved):
        if not depth_to_basements_moved is None:
            if not _utils.is_iterable(depth_to_basements_moved):
                raise Exception('Moved depth to basements set must be iterable objects with the dimensions (m), where m is the number of caverns.')
            depth_to_basements_moved = np.array(depth_to_basements_moved)
            _utils._check_low_high(depth_to_basements_moved, 'depth_to_basements_moved', 0, 100000)
            self._check_dim1D('depth_to_basements_moved', depth_to_basements_moved, dims_equal = 'reservoirs')
            self.set_1D_or_2D('depth_to_basements_moved', depth_to_basements_moved)
    ...
    def set_viscous_tau(self, viscous_tau):
        if not viscous_tau is None:
            ...
            _utils._check_low_high(viscous_tau, 'viscous_tau', -_SubsidenceModelBase.EPSILON, np.inf)
            self._check_dim1D('viscous_tau', viscous_tau, dims_equal = 'reservoirs')
    ...
    def set_volume_change(self, volume_change):
        try:
            ...
            raise Exception(f'Invalid type for input of volume_change: {type(volume_change)}.')
        if len(volume_change.shape) == 2: self._check_dimND('volume_change', volume_change, dims_equal = ['reservoirs', 'timesteps'])
        else: raise Exception(f'Invalid number of dimensions of volume_change: {len(volume_change.shape)}. Use 2 (for reservoir and timesteps).')
        self.drop_from_grid(volume_change, 'volume_change')
        
    def project_compaction(self):
        if not hasattr(self.grid, 'reservoir_mask'):
            raise Exception(f'Model {self.name} has no reservoir mask set. Run model.mask_reservoirs() before projecting compaction.')
    ...
    def calculate_subsidence(self, _print = True):
        ...
        self.grid['subsidence'] = (self.grid['subsidence'] * self.grid.contribution).sum('layer')
    ...
    def calculate_slope(self, reservoir = None, numeric = False, _print = True):
        ...
        super().calculate_slope(reservoir = reservoir, numeric = numeric, _print = False)
        self.grid['slope_x'] = (self.grid['slope_x'] * self.grid.contribution).sum('layer')
        self.grid['slope_y'] = (self.grid['slope_y'] * self.grid.contribution).sum('layer')
        self.grid['slope'] = (self.grid['slope'] * self.grid.contribution).sum('layer')
    ...
    def calculate_concavity(self, reservoir = None, numeric = False, _print = True):
        if _print:
            print(f'Calculating subsidence gradient, model: {self.name}')
        super().calculate_concavity(reservoir = reservoir, numeric = numeric, _print = False)
        self.grid['concavity_xx'] = (self.grid['concavity_xx'] * self.grid.contribution).sum('layer')
        self.grid['concavity_xy'] = (self.grid['concavity_xy'] * self.grid.contribution).sum('layer')
        self.grid['concavity_yx'] = (self.grid['concavity_yx'] * self.grid.contribution).sum('layer')
        self.grid['concavity_yy'] = (self.grid['concavity_yy'] * self.grid.contribution).sum('layer')
        self.grid['concavity'] = (self.grid['concavity'] * self.grid.contribution).sum('layer')
        if _print: print(f'Calculated subsidence gradient, model: {self.name}')
