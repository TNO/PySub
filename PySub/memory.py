...
def _write_variable(Model, var, file):
    if Model.hasattr(var):
        variable = Model[var]
        times = [
            Model.timesteps[t].strftime("%Y-%m-%d")
        ...
        if len(variable.shape) == 4:
            ...
            for r in range(Model.number_of_reservoirs):
                max_value.append(
                    variable.isel(
                        reservoir=r,
                        x=max_indices["x"].isel(reservoir=r).values,
                        y=max_indices["y"].isel(reservoir=r).values,
                    ).values
                )
...
def report(Model):
    ...
    if not _utils.isSubsidenceModel(Model):
        raise Exception(
            f"First argument must be a SubsidenceModel, variable type is: {type(Model)}."
        )
    if not Model.hasattr("timesteps") or not Model.hasattr("reservoirs"):
    ...
    if Model.project_folder.project_folder is not None:
        ...
        print(
            f"Warning, no output file defined for Model {Model.name}, report not saved."
        )
        return
    ...
    with open(fname, "w") as file:
        ...
        bounds_df = pd.Series(
            Model.bounds,
            index=["Lowest x", "lowest y", "highest x", "highest y"],
        )
...
def df_to_text_file(file, list_of_df, titles=None):
    ...
    if titles is not None:
        try:
            ...
            raise Exception(
                "Titles must be a list of strings, or None, equal to the number of entered DataFrames."
            )
    else:
...
def export_grid_to_csv(x, y, values, columns, fname):
    ...
    if len(values.shape) == 2:
        if isinstance(columns, str):
            ...
            if len(columns) != 1:
                raise Exception(
                    f"Incompatible number of columns: {len(columns)}. Must have same length as the 3rd number of dimensions as the values to export.: 1."
                )
        else:
            raise Exception(
                f"Invalid parameter columns: {columns}. Use a string for 2D variables or a list of strings for 2D or 3D variables."
            )
        ...
        if _utils.is_iterable(columns):
            if len(columns) != values.shape[-1]:
                raise Exception(
                    f"Incompatible number of columns: {len(columns)}. Must have same length as the 3rd number of dimensions as the values to export.: {values.shape[-1]}."
                )
        df[columns] = values.reshape(
            (values.shape[0] * values.shape[1], values.shape[2])
        )
    else:
        raise Exception(
            f"Invalid number of dimension for the variable: {len(values.shape)}. Use 2 or 3 dimensions."
        )
...
def to_netcdf(da, filename, epsg=28992):
    ...
    ds = da.transpose(*ds_coords).sortby(ds_coords).to_dataset(name=da.name)
...
def export_ascii(model, variable="subsidence", reservoirs=None, time=None):
    ...
    if _utils.isSubsidenceModel(model):
        ...
        if len(time_index) > 1:
            print(
                f"Warning: Only 1 time index is allowed. Exporting final timestep in entries at: {model.timesteps[time_index[-1]]}"
            )
...
def save(model, path=None):
    ...
    for model in models:
        if model.project_folder.project_folder is not None:
            ...
            print(
                f"Warning: Model {model.name} has no project folder set. Model not saved."
            )
...
def export_tiff(
    ...
    if not _utils.isSubsidenceModel(model):
        raise Exception(
            f"First variable must be of SubsidenceModel type. Is type {type(model)}."
        )
    ...
    if "time" in list(model[variable].coords.keys()):
        ...
        for step in steps:
            ...
            _export_tiff(
                ...
                step,
                reservoir=reservoir,
                fname=f"{file_basename} {time_label}.tif",
                epsg=epsg,
            )
    else:
        _export_tiff(
...
def _export_tiff(
    ...
    if output_file is not None:
        ...
        _shape_utils.save_raster(
            data, model.x, model.y, model.dx, model.dy, epsg, output_file
        )
...
def export_all(model, name=None, epsg=28992):
    ...
    if model.project_folder.project_folder is not None:
            ...
            for data_var in data_variables:
                ...
                if "x" in coordinates:
                    ...
                    if "reservoir" in coordinates:
                        ...
                        _shape_utils.save_raster(
                            total_data, x, y, dx, dy, epsg, total_fname
                        )
                        for i, reservoir in enumerate(model.reservoirs):
                            fname = os.path.join(
                                output_folder,
                                reservoir + "_" + data_var + ".tif",
                            )
                            _shape_utils.save_raster(
                                data[:, :, i], x, y, dx, dy, epsg, fname
                            )
                    else:
                    ...
                    len(coordinates) == 2
                ):  # and there is no x in them (pressures and compaction)
                    data_csv = model.grid[data_var].values
                    data_df = pd.DataFrame(
                        data_csv,
                        index=model.reservoirs,
                        columns=model.timesteps,
                    )
        ...
        _utils.export_df(
            project_df,
            model.project_folder.save_file(f"{model.name}.prj"),
            extension=".prj",
        )
...
def import_bucket_ensemble(import_paths):
    ...
    if isinstance(import_paths, str):
        ...
        raise Exception(
            f"Invalid format for importing files from: {type(import_paths)}."
        )
    buckets = {}
    reservoir_names = {}
    for i, import_path in enumerate(import_paths):
        ...
        ) = import_reservoir_bucket(import_path)
        (
            reservoir_name,
            reservoir_dict,
            dx,
            ...
            timesteps,
        ) = import_reservoir_bucket_from_dfs(
            model_parameters,
            reservoir_parameters,
            reservoir_probabilities,
            pressure_development_df,
        ...
        if reservoir_name in reservoir_names.keys():
            raise Exception(
                f"{reservoir_name} is entered more then once. These two files are describing the same reservoir: \n {import_path} \n {reservoir_names[reservoir_name]}"
            )
    ...
    return (
        ...
        timesteps,
        global_dx,
        global_influence_radius,
        global_compaction_model,
        global_subsidence_model,
    )
...
def import_reservoir_bucket_from_dfs(
    ...
    probability_columns = [
        "Depth to basement probability (factor)",
    ...
    _utils.check_df(
        reservoir_probabilities,
        "Reservoir parameter probability",
    ...
    pressures_dict, timesteps = get_pressures_from_df(
        pressure_development_df, pressure_start_end_df, start, end
    )

    missing_reservoirs = list(
        _utils.a_missing_from_b(
            pressure_probability_df.index.values, pressures_dict.keys()
        )
    )
    missing_pressures = list(
        _utils.a_missing_from_b(
            pressures_dict.keys(), pressure_probability_df.index.values
        )
    )
    ...
    for v in pressure_versions:
        ...
        reservoir_dict["pressures"]["Probabilities"].append(
            pressure_probability_df.loc[v]["Probability"]
        )
    return (
        reservoir_name,
        reservoir_dict,
...
def import_reservoir_bucket_from_excel(import_path):
    ...
    reservoir_parameters = pd.read_excel(
        import_path, sheet_name="Reservoir parameters", index_col=0, header=0
    )

    try:
        pressure_development_df = pd.read_excel(
            import_path,
            sheet_name="Pressure development",
            index_col=0,
            header=0,
        )
    except:
    ...
    try:
        pressure_start_end_df = pd.read_excel(
            import_path, sheet_name="Pressure start-end", index_col=0, header=0
        )
    except:
        pressure_start_end_df = pd.DataFrame()
    pressure_probability_df = pd.read_excel(
        import_path,
        sheet_name="Pressure profile probability",
        index_col=0,
        header=0,
    )
    reservoir_probabilities = pd.read_excel(
        import_path,
        sheet_name="Reservoir parameter probability",
        index_col=0,
        header=0,
    )
    return (
...
def import_reservoir_bucket_from_json(import_path):
    dataframes = json_to_df(
        import_path,
        time_columns={
            "Model parameters": ["Start time", "End time"],
            "Reservoir parameters": [],
...
def import_model_from_dataframes(
    model_parameters,
    reservoir_parameters,
    pressure_start_end_df,
    pressure_development_df,
    ...
    (
        ...
        global_start,
        global_end,
    ) = get_model_parameters_from_df(model_parameters)
    ...
    (
        ...
        compaction_coefficients,
        thickness,
    ) = get_reservoir_parameters_from_df(reservoir_parameters)
...
def get_model_parameters_from_df(
    df,
    columns=[
        "Cell size (m)",
        "Influence radius (m)",
    ...
    try:
        global_start = df["Start time"].values[0]
    except:
        global_start = None
    try:
        global_end = df["End time"].values[0]
    except:
        global_end = None
...
def _pressure_from_start_end(pressure_start_end_df, columns=[], exclude=[]):
    start_value, end_value, start_time, end_time = _values_from_start_end(
        pressure_start_end_df, columns=columns, exclude=exclude
    )

    pressures, timesteps = _utils.profile(
        start_value,
        end_value,
...
def _pressures_from_development(pressure_development_df):
    ...
    timesteps = [
        _utils.convert_to_datetime(pressure_df.columns.values)
        for r in range(len(pressures))
    ]
    return pressures, timesteps
...
def convert_pressure_start_end_to_profile(pressure_start_end_df):
    pressures, timesteps = _pressure_from_start_end(
        pressure_start_end_df, columns=[], exclude=[]
    )
    ...
    for i in range(len(pressures)):
        _pressures, _timesteps = _utils.pad_series(
            timesteps[i],
            pressures[i],
            global_start,
            global_end,
    ...
    for i in range(len(filled_values)):
        pressures.append(
            _utils.interpolate_over_time(
                filled_timesteps[i], filled_values[i], timesteps
            )
        )
...
def convert_pressure(import_path, sheet_name, export_excel=False):
    pressure_start_end_df = pd.read_excel(
        import_path, sheet_name=sheet_name, index_col=None
    )
    ...
    if export_excel:
        ...
        pressure_df.to_excel(
            write_file,
            sheet_name="Pressure_profile",
        )
    return pressure_df


def get_pressures_from_df(
    pressure_development_df, pressure_start_end_df, global_start, global_end
):
    pressure_development_df = pressure_development_df.dropna()
    if (
        pressure_start_end_df.shape[0] == 0
    ):  # sheet pressure start-end has not been filled in, use pressure development
        if (
            pressure_development_df.shape[0] == 0
        ):  # None have been filled in, raise error
            raise Exception(
        ...
        compare_reservoir_names(
            pressure_development_df.index.values,
            pressure_start_end_df.index.values,
            name_df="Pressure development",
            pressure_name_df="Pressure start-end",
        )
...
def get_reservoir_parameters_from_df(reservoir_parameters):
    ...
    compaction_coefficients = _fetch(
        reservoir_parameters, "Compaction coefficient (1/bar)"
    )
...
def compare_reservoir_names(
    reservoir_names,
    pressure_reservoir_names,
    name_df="reservoir parameter",
    pressure_name_df="pressure",
):
    missing_reservoirs = list(
        _utils.a_missing_from_b(reservoir_names, pressure_reservoir_names)
    )
    missing_pressures = list(
        _utils.a_missing_from_b(pressure_reservoir_names, reservoir_names)
    )
    missing = missing_reservoirs + missing_pressures
    if len(missing) > 0:
        raise Exception(
            f"The reservoirs {missing} is missing from either the {pressure_name_df} or {name_df} worksheet."
        )
...
def import_cavern_model(import_path, cumulative_volume=False):
    _, ext = os.path.splitext(import_path)
    if _utils.is_excel_file(import_path):
        return import_cavern_model_from_excel(
            import_path, cumulative_volume=cumulative_volume
        )
    elif ext == "json":
        return import_cavern_model_from_json(
            import_path, cumulative_volume=cumulative_volume
        )
    else:
        raise Exception(f"Files with extension {ext} are not supported.")
...
def import_cavern_model_from_excel(import_path, cumulative_volume=False):
    ...
    cavern_parameters = pd.read_excel(
        import_path, sheet_name="Cavern parameters", index_col=0, header=0
    )

    squeeze_df = pd.read_excel(
        import_path,
        sheet_name="Volume change development",
        index_col=0,
        header=0,
    )
    point_df = pd.read_excel(
    ...
    return import_cavern_model_from_dfs(
        model_parameters,
        cavern_parameters,
        squeeze_df,
        point_df,
...
def import_cavern_model_from_json(import_path, cumulative_volume=False):
    dataframes = json_to_df(import_path, time_columns={"Observations": "Time"})
    model_parameters = dataframes["Model parameters"]
...
def import_cavern_parameters_from_df(cavern_parameters):
    ...
    depth_to_basements_moved, viscous_tau = cavern_parameters[
...
def import_cavern_model_from_dfs(
    model_parameters,
    cavern_parameters,
    squeeze_df,
    point_df,
    ...
    (
        ...
        start_time,
        end_time,
    ) = get_model_parameters_from_df(
        model_parameters,
        columns=["Cell size (m)", "Influence radius (m)", "Subsidence model"],
    )
    (
        reservoir_names,
        coordinates,
        ...
        knothe_angles,
        lengths,
    ) = import_cavern_parameters_from_df(cavern_parameters)

    compare_reservoir_names(reservoir_names, squeeze_df.index)
    timesteps, cum_squeeze = import_squeeze_volumes_from_df(
        squeeze_df, cumulative_volume=cumulative_volume
    )
...
def build_suite(
    ...
    if isinstance(import_path, str):
        ...
        pf = (
            ...
            else os.path.dirname(import_path)
        )
        ...
        (
            names,
            multi_dx,
            multi_influence_radius,
            multi_compaction_model,
            multi_subsidence_model,
            multi_tau,
            multi_knothe_angles,
            reservoir_names,
            multi_shapefile_paths,
            multi_depths,
            multi_depth_to_basements,
        ...
        suite.set_compaction_model_parameters(
            multi_compaction_model,
            multi_compaction_coefficients,
            multi_tau,
            multi_reference_stress_rates,
        ...
        suite.set_subsidence_model_parameters(
            multi_subsidence_model,
            knothe_angles=multi_knothe_angles,
        ...
        suite.set_bounds(
            bounds=bounds, all_same_bounds=all_same_bounds
        )  # or: Model.set_bounds(bounds = None) to build from shapefiles, if set
        ...
        suite_name = (
            name
            if name is not None
            else os.path.basename(os.path.dirname(import_path[0]))
        )
        ...
        for model_name, model_path in zip(model_names, import_path):
            model = build_model(
                model_path,
                name=model_name,
                project_folder=suite.project_folder.project_folder,
            )
...
def seperate_models_from_df(
    ...
    for model in model_parameters.index:
        ...
        model_reservoir_parameters = reservoir_parameters[
            reservoir_parameters["Name"] == model
        ]
        ...
        try:
            shapefile_paths.append(
                model_reservoir_parameters["Shapefile location"].values
            )
        except:
        ...
        try:
            density.append(
                model_reservoir_parameters[
                    "Average density above reservoir (kg/³)"
                ].values
            )
        except:
            density = None
        try:
            ...
            cmref = None
        try:
            b.append(model_reservoir_parameters["b"].values)
        except:
            b = None
...
def build_model_from_xarray(xarray, name, project_folder=None):
    ...
    model.number_of_timesteps = len(_xarray.time)
    model.number_of_reservoirs = len(_xarray.reservoir)
...
def load_observation_points(
    ...
    if not os.path.isfile(import_path):
        raise Exception(
            "The path should be a string that directs to a json or Excel file."
        )
    ...
    observation_points = _Points.load_observation_points_from_df(
        ...
        lower_error_column=lower_error_column,
        upper_error_column=upper_error_column,
    )
