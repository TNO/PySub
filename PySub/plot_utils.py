...
def raise_invalid_color_exception(number_of_groups):
    ...
    """
    raise Exception(f'Invalid entry for group coloring. It should be a list of RGBA colors with the same length as the number of groups ({number_of_groups}).')

def get_colors_grouped(plot_objects, groups, cmap = 'gist_rainbow', group_colors = None):
    ...
    """
    unique_groups, index, counts = np.unique(groups, return_index = True, return_counts = True)
...
def plot_probability_distribution(Model, values, probabilities = None, unit = 'cm', figsize = (8, 6), title = None, final = True, fname = '', svg = False, **kwargs):
    """
    ...
    if _utils.is_iterable(values):
        ...
        ax.plot(sorted_p_df['values'].values, sorted_p_df['cumsum'].values, **kwargs)
...
def add_colorbar(ax, contours, fig, contour_lines = None, pad = 0.01, colorbar_kwargs = {}):
    ...
    cax.set_position([posn.x0 + posn.width + pad, posn.y0,
                         colorbar_width, posn.height])
...
def add_scalebar(ax, extent):
    ...
    if x_size == 0:
        raise Exception('The extent of the figure is 0. No figure can be displayed')
    ...
    scalebar = AnchoredSizeBar(ax.transData,
                           scale_bar_size, f'{scale_bar_text} {unit}', 'lower right', 
                           pad=0.1,
                           color='k',
                           frameon=False,
...
def add_title(ax, title):
    ...
    if title is not None:
        if isinstance(title, str):
            ...
            raise Exception(f'Invalid object type for title: {type(title)}. Use a string.')
            
def add_custom_legend(ax, types = [], kwargs = [], labels = [], loc='center left', bbox_to_anchor=(1., 0.5)):
    ...
    ax.legend(handles=handles, labels =labels, loc=loc, bbox_to_anchor=bbox_to_anchor)
        
def _dummy_mappable(x, y, contour_levels, contour_steps, contourf_kwargs = {}, contour_kwargs = {}):
    ...
    dummy_mappable = dummy_axis.contourf(x, y, data, levels = contour_levels, **contourf_kwargs)
    dummy_mappable_lines = dummy_axis.contour(x, y, data, levels = contour_levels, **contour_kwargs)
    plt.close()
    return dummy_mappable, dummy_mappable_lines

def add_contours(ax, x, y, data, contour_levels, contour_steps, 
                 contourf_kwargs = {}, contour_kwargs = {}, clabel_kwargs = {}):
    ...
    """
    contour_levels = set_contour_levels(data, contour_levels = contour_levels, contour_steps = contour_steps)
    ...
    if contour_levels is None:
        ...
        if len(contour_levels) > 100:
            warn(f'Warning: {len(contour_levels)} contours found. This can take very long.')
        if np.min(data) < -contour_steps or np.max(data) > contour_steps: 
            filled_contours = ax.contourf(x, y, data, levels = contour_levels, **contourf_kwargs)
            contour_lines = ax.contour(x, y, data, levels = contour_levels, **contour_kwargs)
            ax.clabel(contour_lines, contour_levels, **clabel_kwargs)
        
        filled_contours, contour_lines = _dummy_mappable(x, y, contour_levels, contour_steps, 
                                                         contourf_kwargs = contourf_kwargs, contour_kwargs = contour_kwargs)
...
def add_geometries(ax, geometries = [], scatter_kwargs = {}, shape_kwargs = {}, raster_kwargs = {}):
    ...
    """
    types = [geom.type if hasattr(geom, 'type') else None for geom in geometries]
    wrong = [t is None for t in types]
    if any(wrong):
        raise Exception(f'Invalid type of geometry: {type(np.array(geometries)[wrong][0])}.')
    unique_types, inverse, count = np.unique(types, return_counts = True, return_inverse = True)
    count_dict = {t: c for t, c in zip(unique_types, count)}
    index_dict = {t: np.where(inverse == i)[0] for i, t in enumerate(unique_types)}
    for t in unique_types:
        for i, geom_index in enumerate(index_dict[t]):
            ...
            individual_kwargs = _utils.pick_from_kwargs(kwargs, i, (int(count_dict[geom.type])))
...
def add_raster(ax, raster, block_value = 0, raster_kwargs = {}):
    if not isinstance(raster, dict):
        raise Exception('Add_raster takes an axis and dictionary as argument. The dictionary should have the keys "X", "Y", "mask".')
    ...
    if block_value is not None:
        masked_raster = np.ma.masked_where(raster['mask'] == block_value, raster['mask'])
...
def add_annotations(ax, labels, points, annotation_kwargs = {}):
    annotations = [ax.annotate(label, point, path_effects = WHITE_SHADOW, **annotation_kwargs)
                    for label, point in zip(labels, points)]
    adjust_text(annotations, arrowprops=dict(arrowstyle="-", color='k', lw=0.5))
    ...
    if line is not None:
        if _utils.is_iterable(line) and type(line) != dict:
            ...
            if line.shape[1] == 2:
                ...
                raise(Exception('Line object can be dict or list of sets with 2 nummerical entries.'))
        elif type(line) == dict:
            labels = line.keys()
            points = list(line.values())
            xs, ys = np.array(list(line.values())).T
            ax.plot(xs, ys, 'ro-')
            add_annotations(ax, labels, points, annotation_kwargs = annotation_kwargs)
        else:
            raise(Exception('Line object can be dict or list with 2x2 shape.'))
    if remember is None:
        return fig, ax
            
def add_points(ax = None, points = None, labels = None, scatter_kwargs = {}, annotation_kwargs = {}):
        ...
        if _utils.is_iterable(points) and type(points) != dict:
            ...
            if points.shape[1] == 2:
                if 'zorder' not in scatter_kwargs.keys():
                    ...
                    scatter_kwargs_adjusted = {k: v for k, v in scatter_kwargs.items() if k != 'zorder'}
                ax.scatter(points.T[0], points.T[1], zorder = zorder, **scatter_kwargs_adjusted)
            else:
                raise(Exception('point object can be dict or list of sets with 2 nummerical entries.'))
            ...
            if labels is not None:
                add_annotations(ax, labels, p, annotation_kwargs = annotation_kwargs)
            
        else:
            raise(Exception('point object can be dict or list with 2x2 shape.'))
    if remember is None:
        return fig, ax
    
def add_errorbar(ax = None, x = None, y = None, upper_limits = None, lower_limits = None, c = None, label = '_nolabel_', errorbar_marker = '_', errorbar_kwargs = {}):
    ...
    if np.logical_not(np.array(plot_test)).all():
        l1, cap1, _ = ax.errorbar(x, y, yerr=upper_limits, uplims=True, lolims=False, c = c, label = label, **errorbar_kwargs)
        for cap in cap1: cap.set_marker(errorbar_marker)
        l2, cap2, _ = ax.errorbar(x, y, yerr=lower_limits, uplims=False, lolims=True, c = c,  label = '_nolabel_', **errorbar_kwargs)
        for cap in cap2: cap.set_marker(errorbar_marker)
...
def shiftedColorMap(cmap, start=0, midpoint=0.5, stop=1.0, levels = [0], alpha = 1, midpoint_alpha = 0):
    ...
    if len(levels) == 1:
        ...
        for ri, si in zip(reg_index, shift_index):
            ...
            if midpoint+(1/len(levels))>si>midpoint-(1/len(levels)):
                a = midpoint_alpha
            else:
                a = alpha
            cdict['alpha'].append((si, a, a))
    ...
    if 'cmap' in kwarg_dict.keys():
        if kwarg_dict['cmap'] is not None:
            ...
            if type(kwarg_dict['cmap']) == str:
                ...
                shifted_cmap = shiftedColorMap(cmap, start = 0, midpoint = midpoint, stop = 1, levels = levels, alpha = alpha)
                kwarg_dict['cmap'] = shifted_cmap
            elif type(kwarg_dict['cmap']) == LinearSegmentedColormap:
                if kwarg_dict['cmap'].name.startswith('shifted_'):
                    shifted_cmap = shiftedColorMap(cm.get_cmap(kwarg_dict['cmap'].name[8:]), start = 0, midpoint = midpoint, stop = 1, levels = levels, alpha = alpha)
                    kwarg_dict['cmap'] = shifted_cmap
                else:
                    shifted_cmap = shiftedColorMap(kwarg_dict['cmap'], start = 0, midpoint = midpoint, stop = 1, levels = levels, alpha = alpha)
                    kwarg_dict['cmap'] = shifted_cmap
            else: 
                raise Exception('Invalid colormap type: {type(type(kwarg_dict["cmap"]))}')


def set_contour_levels(data = [0], contour_levels = None, contour_steps = 0.01, drop_value = 0):
    ...
    """
    if contour_levels is None:
        contour_levels = np.array(_utils.stepped_space(np.min(data), np.max(data), contour_steps))
    elif not _utils.is_iterable(contour_levels):
        raise Exception(f'Invalid contour level type: {type(contour_levels)}. Use list or other itterable to define contour levels explicitly, or None to define it on value range.')
    ...
    if len(contour_levels) == 0:
        ...
        if max(contour_levels) < contour_steps:
            ...
            contour_levels = np.array(clist)
        contour_levels = contour_levels[np.logical_not(np.isclose(contour_levels, drop_value, atol = 1e-14))]
...
def set_defaults(input_dict, defaults = {}):
    ...
    if type(input_dict) != dict or type(defaults) != dict:
        raise(TypeError('The input values for set_defaults need to be dictionaries'))
    output_dict = input_dict.copy()
    if 'colors' in input_dict.keys():
        try: del defaults['c']
        except: pass
        try: del defaults['cmap']
        except: pass
    if 'cmap' in input_dict.keys():
        try: del defaults['c']
        except: pass
        try: del defaults['colors']
        except: pass
...
def get_cross_section(x, y, data, line, num):
    ...
    for l in range(number_of_points - 1):
        ...
        for i in range(len(data)):
            segment_values = _utils.get_values_cross_section(_A, _B, data[i].T, num = num)
            values[i][l*num:(l+1)*num] = segment_values
            
    return values, distances, np.array(inflection_distances)

def add_cross_section(ax, x, y, data, line, name = '_nolegend_',
                      inflection_point_names = None, c = None,                        
                      num = 1000, 
                      plot_kwargs = {},
                      annotation_kwargs = {}):
    ...
    if _utils.is_iterable(line):
        ...
        if inflection_point_names is None:            
            ...
            if len(inflection_point_names) != number_of_points:
                warn('Warning: Invalid number of names for inflection point. The entry must have the same length as the amount of points in the line list/dict.')
                for i, point in enumerate(line):
                    line_dict[string.ascii_uppercase[i % 26]] = point
            else:
                for i, point in enumerate(line):
    ...
    values, distances, inflection_distances = get_cross_section(x, y, data, line.values(), num)
    
    for l in range(number_of_points - 1):
        if l == 0:
            labels = [list(line.keys())[l]]
            points = [(inflection_distances[0], 0)]
            ax.axvline(x = inflection_distances[0], c = 'k', linestyle = 'dashed', alpha = 0.25, label = '_nolegend_')
            add_annotations(ax, labels, points, annotation_kwargs = annotation_kwargs)
        labels = [list(line.keys())[l + 1]]
        points = [(inflection_distances[l + 1], 0)]
        ax.axvline(x = inflection_distances[l + 1], c = 'k', linestyle = 'dashed', alpha = 0.25, label = '_nolegend_')
        add_annotations(ax, labels, points, annotation_kwargs = annotation_kwargs)
...
def set_ylim(ax, ylim = None, y_axis_exageration_factor = 1, unit_in = 'm', unit_out = 'm'):
   ...
   ax : matplotlib axes object
    ...
    """
    if ylim is None:
        ax.set_ylim([np.min(ax.get_ylim()) * y_axis_exageration_factor , np.max(ax.get_ylim())])
    else:
        ylim = _utils.convert_SI(np.array(ylim), unit_in, unit_out)
        ax.set_ylim(ylim)
    
def plot_2D_data(extent, x = None, y = None, data = None, 
                 figsize = (8, 8),
    ...
    if np.logical_not(np.array(plot_test)).all():
        contours, contour_lines = add_contours(ax, x, y, data, contour_levels = contour_levels, 
                          contour_steps = contour_steps, 
                          contourf_kwargs = contourf_kwargs, 
                          contour_kwargs = contour_kwargs, 
                          clabel_kwargs = clabel_kwargs)
        if contours is not None: 
            add_colorbar(ax, contours, fig, contour_lines = contour_lines, colorbar_kwargs = colorbar_kwargs)
    add_north_arrow()
    add_scalebar(ax, extent)
    add_title(ax, title)
    return fig, ax

def plot_3D_data(extent, x, y, data, 
                 figsize = (8, 8),
                 contour_levels = None, contour_steps = 0.01, title = None,
                 geometries = [],
                 basemap = True, 
    ...
    if sum(check) > 0:
        raise Exception(f'Parameters {parameter_name[check]} do not have the same number of first axis as the data. The first axis must have the same length as the number of plotted figures.')
    contours, contour_lines = [], []
    
    for i, d in enumerate(data):
        try:
            ax = fig.add_subplot(number_of_rows, number_of_columns, i+1, projection = ccrs.epsg(epsg))
        ...
        add_geometries(ax, geometries = geometries[i],
                       scatter_kwargs = scatter_kwargs, shape_kwargs = shape_kwargs, raster_kwargs = raster_kwargs)
        _contours, _contour_lines = add_contours(ax, x[i], y[i], data[i], 
    ...
    for i, ax in enumerate(fig.axes):
        if contours[i] is not None:
            add_colorbar(ax, contours[i], fig, contour_lines = contour_lines[i], colorbar_kwargs = colorbar_kwargs)
        
    fig.set_size_inches(figsize)
    return fig, fig.axes


def resolve_title(Model, title, variable, unit, step = -1, model = None, Suite = None):
    if title is None:
...
def seperate_colors_from_dict(plot_kwargs, number_of_entries):
    ...
    if _utils.is_iterable(c):
        if len(c) != number_of_entries:
            raise Exception(f'The number of arguments for color or c ({len(c)}) do not equal the amount of entries: {number_of_entries}.')
    else:
...
def get_2D_data_from_model(Model, reservoir = None, time = -1, variable = 'subsidence', unit = 'cm'):
    ...
    if not ('x' in data_coords and 'y' in data_coords):
        raise Exception('Model variable needs to have the "x" and "y" dimensions.')
...
def plot_subsidence_model(Model, reservoir = None, time = -1, buffer = 0, 
    ...
    scatter_kwargs = set_defaults(scatter_kwargs, defaults = Model.scatter_defaults)
    ...
    if _utils.isSubsidenceModel(Model):
        if not Model.hasattr(variable):
            calculator = f'calculate_{variable}' if not variable.endswith(('_x', '_y')) else f'calculate_{variable[:-2]}'
            if not hasattr(Model, calculator):
                calculator = f'set_{variable}' if not variable.endswith(('_x', '_y')) else f'set_{variable[:-2]}'
                if not hasattr(Model, calculator):
                    raise Exception(f'No {variable} in model.')
            raise Exception(f'No {variable} data available, run {calculator} before attempting to plot')
        
        reservoir_index = reservoir_entry_to_index(Model, reservoir)
        step = time_entry_to_index(Model, time, _2d = True)
        data = get_2D_data_from_model(Model, reservoir = reservoir, time = time, variable = variable, unit = unit)
        ...
        if (plot_reservoir_shapes and Model.hasattr('shapes')):
            geometries = [Model.shapes[r] for r in reservoir_index] + additional_shapes
        else:
        ...
        raise Exception('Invalid input type for model: {type(Model)}. SubsidenceModel object is required.')
        
def plot_subsidence_suite(Suite, reservoir = None, time = -1, buffer = 0, model = None,
                          variable = 'subsidence',
                          unit = 'cm',
                          plot_reservoir_shapes = True,
                          additional_shapes = [],
    ...
    if _utils.isSubsidenceSuite(Suite):
        shape_kwargs = set_defaults(shape_kwargs, defaults = Suite.shape_defaults)
        contourf_kwargs = set_defaults(contourf_kwargs, defaults = Suite.contourf_defaults)
        contour_kwargs = set_defaults(contour_kwargs, defaults = Suite.contour_defaults)
        clabel_kwargs = set_defaults(clabel_kwargs, defaults = Suite.clabel_defaults)
        colorbar_kwargs = set_defaults(colorbar_kwargs, defaults = Suite.colorbar_defaults)
        raster_kwargs = set_defaults(raster_kwargs, defaults = Suite.raster_defaults) 
        scatter_kwargs = set_defaults(scatter_kwargs, defaults = Suite.scatter_defaults) 
        ...
        for i, Model in enumerate(Suite._models):
            if i in model:
                if not Model.hasattr(variable):
                    raise Exception(f'The variable {variable} has not been set/calculated for model {Model.name}.')
                reservoir_index = reservoir_entry_to_index(Model, reservoir)
                step = Model.time_label_to_int(time)
                
                model_data = get_2D_data_from_model(Model, reservoir = reservoir_index, time = step, variable = variable, unit = unit)
                x.append(Model.x)
                y.append(Model.y)
        
                if (plot_reservoir_shapes and Model.hasattr('shapes')):
                    model_geometries = [Model.shapes[r] for r in reservoir_index] + additional_shapes
                else:
                    model_geometries = additional_shapes
                geometries.append(model_geometries)
                data.append(model_data)
            
                titles.append(resolve_title(Model, title, variable, unit, Suite = Suite, model = i, step = step))
        if contour_levels is None:
            contour_steps = _utils.convert_SI(np.array(contour_steps), 'm', unit)  
        else:
            contour_levels = _utils.convert_SI(np.array(contour_levels), 'm', unit)
        ...
        raise Exception('Invalid input type for model: {type(Suite)}. ModelSuite object is required.')
    
    
def plot_subsidence(Model, reservoir = None, time = -1, buffer = 0, model = None,
                    variable = 'subsidence',
                    unit = 'cm',
                    plot_reservoir_shapes = True, 
                    additional_shapes = [],
    ...
    if _utils.isSubsidenceModel(Model):
        return plot_subsidence_model(Model, reservoir = reservoir, 
                                     time = time, 
                                     buffer = buffer,
                                     variable = variable,
                                     unit = unit,
    ...
    if _utils.isSubsidenceSuite(Model):
        return plot_subsidence_suite(Model, reservoir = reservoir, model = model,
                                     time = time, 
                                     buffer = buffer, 
                                     variable = variable,
                                     unit = unit,
...
def add_errorbars(ax = None, observations = None, unit = 'cm', errorbar_kwargs = {}):
    ...
    if not str(type(observations)) == "<class 'PySub.Points.ObservationCollection'>":
        raise Exception(f'Invalid observation type: {type(observations)}. No Points.ObservationCollection to plot')
        if remember is None: return fig, ax
        else: return
    c, adjusted_errorbar_kwargs = seperate_colors_from_dict(errorbar_kwargs, len(np.unique(observations.names)))
...
class AskForLine():
    ...
    def close(self):
        if len(self.point_names) < 2:
            messagebox.showinfo('Line message', "Not enough points selected to continue. Select at least 2.")
        else:
            self.root.destroy()
            plt.close(self.fig)
...
def plot_cross_section(Model, lines = None, variable = 'subsidence', reservoir = None, time = -1, model = None,
    ...
    if not Model.hasattr(variable):
        warn(f'Warning, no {variable} has been calculated. No cross section has been made.')
        return
    ...
    if _utils.isSubsidenceModel(Model):
        steps = time_entry_to_index(Model, time)
        # reservoir_index = reservoir_entry_to_index(Model, reservoir)
        data = get_transient_data_from_model(Model, reservoir = reservoir, time = time, variable = variable, unit = unit)
            
        legend_labels = time_to_legend(time, Model)
                
        fig, ax = plt.subplots()
        c, adjusted_plot_kwargs = seperate_colors_from_dict(plot_kwargs, len(steps))
        add_cross_section(ax, Model.x, Model.y, data, line_dict.values(),
                          name = legend_labels,
                          inflection_point_names = None, c = c,
                          num = num, 
    ...
    if _utils.isSubsidenceSuite(Model):
        ...
        for i, m in enumerate(Model._models):
            if i in model:
                ...
                c, adjusted_plot_kwargs = seperate_colors_from_dict(plot_kwargs, len(steps))
                data = get_transient_data_from_model(m, reservoir = reservoir, time = steps, variable = variable, unit = unit)
                    
                adjusted_plot_kwargs = set_defaults({'linestyle': _line_style(counter % 10)}, adjusted_plot_kwargs)
                
                add_cross_section(ax, m.x, m.y, data, line_dict.values(), name = _legend_labels,
                                  inflection_point_names = None, c = c,
                                  num = num, 
                                  plot_kwargs = adjusted_plot_kwargs,
                                  annotation_kwargs = annotation_kwargs)
...
def plot_reservoirs(Model, reservoir = None, model = None,
    ...
    if _utils.isSubsidenceModel(Model):
        return plot_reservoirs_model(Model, reservoir = reservoir, 
        ...
        return plot_reservoirs_suite(Model, reservoir = reservoir, model = model,
                          buffer = buffer, annotate = annotate, 
                          plot_reservoir_shapes = plot_reservoir_shapes,
                          additional_shapes = additional_shapes, additional_labels = additional_labels,
                          service = service, layer = layer, figsize=figsize, epsg = epsg,
...
def plot_reservoirs_suite(Suite, reservoir = None, model = None,
                    buffer = 0, annotate = True, 
                    plot_reservoir_shapes = True,
                    additional_shapes = [], additional_labels = [],
                    service = 'opentopo', layer = 'opentopoachtergrondkaart', figsize=(8, 8), epsg = 28992, 
    ...
    for i, m in enumerate(Suite._models):
        if i in model_index:
            if m.hasattr('shapes'):
                ...
                add_geometries(ax, geometries = geometries,
                               scatter_kwargs = scatter_kwargs, shape_kwargs = shape_kwargs, 
                               raster_kwargs = raster_kwargs)
                
    if annotate:
        for m in model_index:
            reservoir_index = reservoir_entry_to_index(Suite._models[m], reservoir)
            shapes = [Suite[m].shapes[i] for i in reservoir_index] + additional_shapes
            _points = [s.midpoint for s in shapes]
            _labels = [Suite[m].reservoirs[i] for i in reservoir_index] + list(additional_labels)
            add_annotations(ax, _labels, _points, annotation_kwargs = annotation_kwargs)
...
def savefig(Model_or_Suite, fname, svg = False):
    if fname:
        if not hasattr(Model_or_Suite, 'project_folder'):
            warn(f'Warning: no project folder has been defined for this Model/Suite {Model_or_Suite.name}. Figure {fname} has not been saved')
            return
...
def plot_reservoirs_model(Model, reservoir = None, 
                          buffer = 0, annotate = True, 
                          plot_reservoir_shapes = True,
                          additional_shapes = [], additional_labels = [],
                          service = 'opentopo', layer = 'opentopoachtergrondkaart', figsize=(8, 8), epsg = 28992,
    ...
    if annotate:
        ...
        labels = [r for i, r in enumerate(Model.reservoirs) if i in reservoir_index] + list(additional_labels)
...
def plot_points_on_map(Model, points = None, labels = None, reservoir = None, time = -1, buffer = 0,
                       unit = 'cm',
    ...
    if (show_data and hasattr(Model.grid, 'subsidence')):
        ...
        fig, ax = plot_reservoirs(Model, reservoir = reservoir, 
                                  plot_reservoir_shapes = plot_reservoir_shapes,
        ...
        if hasattr(points, 'x'):
            ...
            if labels is not None:
                ...
                if _utils.is_iterable(x) and _utils.is_iterable(y):
                    names = np.array([(ix, iy) for (ix, iy) in zip(x, y)]).astype(str)
                else:
                    names = [str(x), str(y)]
        else:
            if not _utils.is_iterable(points):
                raise Exception(f'Invalid format for points: {points}. Enter a Point(Collection), Observation(Collection or Point), or numpy array with 2 dimensions, (m, 2) where m is the number of points and the 2 are the x- and y-coordinates.')
            points = np.array(points)
            if len(points.shape) == 1:
                points = points[None, :]
                
            if len(points.shape) != 2 or points.shape[1] != 2:
                raise Exception(f'Invalid format for points: {points}. \n Enter a Point(Collection), Observation(Collection or Point), or numpy array with 2 dimensions, (m, 2) where m is the number of points and the 2 are the x- and y-coordinates.')
            x = points[:, 0]
            y = points[:, 1]
            if labels is not None:
                if _utils.is_iterable(labels):
                    if not len(labels) == len(x):
                        raise Exception('Point labels have length {len(labels)}, which does not equal the amount of points: {len(x)}')
                    else:
                        names = labels
                else:
                    raise Exception('Invalid type of labels, enter an iterable.')
            else:
    ...
    add_points(ax = ax, points = point, labels = annotations, scatter_kwargs = scatter_kwargs, annotation_kwargs = annotation_kwargs)
...
def add_subsidence_points(ax, Model, points = None, reservoir = None, 
                          unit = 'cm',
                          plot_kwargs = {}):
    ...
    if 'label' in plot_kwargs.keys():
        ...
        if _utils.is_iterable(label):
            if len(label) != Model.number_of_points:
                raise Exception('Invalid number of entries for labels. A list of labels needs to be equal to the amount of points in a model.')
        elif type(label) == str:
    ...
    c, adjusted_plot_kwargs = seperate_colors_from_dict(plot_kwargs, Model.number_of_points)
    if len(c) != Model.number_of_points:
        c = [None for _ in Model.number_of_points]
        warn('Warning: invalid color keyword argument added. Use a list with the length of the points in each model.')
    
    
    for pi in point_index:
        values = Model.point_subsidence.isel(reservoir = reservoir).sum(dim = 'reservoir').isel(points = pi).values
        values = _utils.convert_SI(values, 'm', unit)
        plt.plot(Model.timesteps, values, 
                 c = c[pi], label = label[pi], **adjusted_plot_kwargs)

   
def plot_subsidence_points(Model, points = None, reservoir = None, model = None,
                           unit = 'cm',
                           title = None,
                           y_axis_exageration_factor = 2,
                           ylim = None,
    ...
    fig.set_size_inches(figsize)
    if _utils.isSubsidenceModel(Model):
        if not (hasattr(Model.grid, 'point_subsidence') and
            Model.grid.point_subsidence.shape[0] != 0):
            warn('Warning: No subsidence has been calculated for points in this model.')
            if final: 
                return
            else: 
                return fig, ax            
        add_subsidence_points(ax, Model, points = points, reservoir = reservoir, 
                           unit = unit,
        ...
        for i, m in enumerate(Model._models):
            if i in model:         
                if not (hasattr(m.grid, 'point_subsidence') and
                    m.grid.point_subsidence.shape[0] != 0):
                    warn(f'Warning: No subsidence has been calculated for points in model {m.name}.')
                adjusted_kwargs = set_defaults({'linestyle': _line_style(counter % 10)}, plot_kwargs)
                add_subsidence_points(ax, m, points = points, reservoir = reservoir,
                                   unit = unit,
                                   plot_kwargs = adjusted_kwargs)
                counter += 1
    
    else:
        raise Exception(f'Invalid type: {type(Model)}. Use SubsidenceModel or ModelSuite object.')
...
def add_subsidence_at_observation_points(ax, Model, 
                                         observations = None, reservoir = None,
                                         unit = 'cm',
                                         plot_kwargs = {}):
    
    if _utils.isSubsidenceModel(Model):
        if (not Model.hasattr('observation_points') or
            Model.number_of_observation_points == 0):
            warn('Warning: No observation points to plot.')
            return plt.subplot()
        
        if (Model.hasattr('observation_subsidence') and
            Model.number_of_observation_points != 0):
            reservoir = reservoir_entry_to_index(Model, reservoir)
            
            type_observations = type(observations)
            if observations is None:
                observations_index = list(range(Model.number_of_observation_points))
            elif _utils.is_iterable(observations):
                observations_index = [Model.observation_label_to_int(r) for r in observations]
            elif type_observations == int or type_observations == str:
                observations_index = [Model.observation_label_to_int(observations)]
            
            c, adjusted_plot_kwargs = seperate_colors_from_dict(plot_kwargs, len(observations_index)) 
            if 'label' in plot_kwargs.keys():
                label = plot_kwargs['label']
                if _utils.is_iterable(label):
                    pass
                elif type(label) == str:
                    label = [label for _ in range(Model.number_of_observation_points)]
                elif type(label) == int:
                    label = [str(label) for _ in range(Model.number_of_observation_points)]
                else:
                    raise Exception('Invalid labels')
                del plot_kwargs['label']
            else:
                label = np.array(Model.observation_points.names)[observations_index]
            label = [str(l) + ' ' + Model.name for l in label]
            for pi, p in enumerate(observations_index):
                values = Model.observation_subsidence.isel(reservoir = reservoir).sum(dim = 'reservoir').isel(observations = p).values
                values = _utils.convert_SI(values, 'm', unit)
                ax.plot(Model.timesteps, values, c = c[pi],
                         label = label[pi], **adjusted_plot_kwargs)
    else:
        raise Exception(f'{type(Model)} is invalid entry.')



def plot_subsidence_observations(Model, observations = None, reservoir = None, model = None,
                                 unit = 'cm',
                                 title = None,
                                 y_axis_exageration_factor = 2,
                                 ylim = None,
    ...
    """
    plot_kwargs = set_defaults(plot_kwargs, defaults = Model.plot_defaults)
    errorbar_kwargs = set_defaults(errorbar_kwargs, defaults = Model.errorbar_defaults)
    ...
    if _utils.isSubsidenceModel(Model): 
        if observations is None:
            ...
            if _utils.is_iterable(observations):
                plot_observations = _Points.ObservationCollection(Model.observation_points[observations])
            else:
                plot_observations = _Points.ObservationCollection([Model.observation_points[observations]])
        add_subsidence_at_observation_points(ax, Model, 
                               observations = observations, reservoir = reservoir,
                               unit = unit,
                               plot_kwargs = plot_kwargs)
        
        
                                                               
        add_errorbars(ax = ax, observations = plot_observations, unit = unit, errorbar_kwargs = errorbar_kwargs)
    
    elif _utils.isSubsidenceSuite(Model):
        model = Model.model_label_to_index(model)
        
        counter = 0
        c, pre_adjusted_kwargs = seperate_colors_from_dict(plot_kwargs, len(np.unique(Model.unique_observations(observations = observations).names)))
        for i, m in enumerate(Model._models):
            if i in model:
                adjusted_plot_kwargs = set_defaults({'linestyle': _line_style(counter % 10)}, pre_adjusted_kwargs)
                adjusted_plot_kwargs['c'] = c[[i for i, e in enumerate(np.unique(Model.unique_observations().names)) if e in m.observation_points.names]]
                add_subsidence_at_observation_points(ax, m, 
                                        observations = observations, reservoir = reservoir,
                                        unit = unit, plot_kwargs = adjusted_plot_kwargs)
                counter += 1
        
        add_errorbars(ax = ax, observations = Model.unique_observations(observations = observations), unit = unit, errorbar_kwargs = errorbar_kwargs)
...
def plot_timeseries(Model, points = None, 
                    variable = 'subsidence', 
                    reservoir = None, 
                    model = None, 
                    unit = 'cm', 
    ...
    if mode.lower() != 'max':
        if points is None:
            raise Exception('The point values must be entered when not using mode = "max".')
    
        points, x, y = _utils.point_or_points(points)
        number_of_points = len(points)
        
        if _utils.isSubsidenceModel(Model):
            if not Model.hasattr(variable): raise Exception(f'No variable {variable} to plot timeseries from.')
            c, adjusted_plot_kwargs = seperate_colors_from_dict(plot_kwargs, number_of_points) 
            reservoir_index = reservoir_entry_to_index(Model, reservoir)
        ...
        if _utils.isSubsidenceSuite(Model):
            ...
            for j, p in enumerate(points):
                ...
                for i, m in enumerate(Model._models):
                    if i in model:
                        if not m.hasattr(variable): raise Exception(f'No variable {variable} to plot timeseries from.')
                        adjusted_kwargs = set_defaults({'linestyle': _line_style(counter % 10)}, adjusted_plot_kwargs)
                        reservoir_index = reservoir_entry_to_index(m, reservoir)
        ...
        if _utils.isSubsidenceModel(Model):
            ...
            data = Model.get_timeseries(variable = variable, reservoir = reservoir_index, x = x, y = y)
        ...
        if _utils.isSubsidenceSuite(Model):
            ...
            model = Model.model_label_to_index(model)  
            c, adjusted_plot_kwargs = seperate_colors_from_dict(plot_kwargs, Model.number_of_models) 
            counter = 0
            ...
            for i, m in enumerate(Model._models):
                if i in model:
                    ...
                    data = m.get_timeseries(variable = variable, reservoir = reservoir_index, x = x, y = y)
...
def plot_min_mean_max(Suite, point = None, reservoir = None, model = None, unit = 'cm', title = None,
                    y_axis_exageration_factor = 2, ylim = None, figsize = (8, 8), 
                    final = True, fname = 'min_mean_max', svg = False,
                    mode = 'coord', 
                    legend = True, 
                    horizontal_line = None,
                    plot_kwargs = {}, fill_between_kwargs = {}):
    ...
    fill_between_kwargs = set_defaults(fill_between_kwargs, defaults = Suite.fill_between_defaults)
    
    if not _utils.isSubsidenceSuite(Suite):
        raise Exception(f'Invalid input type {type(Suite)} for first input argument. Must be ModelSuite.')
    ...
    if mode.lower() != 'max':
        if point is None:
            ...
            raise Exception('The point values are required to be a tuple of an x- and y-coordinate.')
        elif len(point) != 2:
            raise Exception('The point values are required to be a tuple of an x- and y-coordinate.')
        else:
            x, y = point
    ...
    _min, _mean, _max = Suite.get_subsidence_spread(x, y, reservoir = reservoir, model = model)
    _min = _utils.convert_SI(_min, 'm', unit)
    _mean = _utils.convert_SI(_mean, 'm', unit)
    _max = _utils.convert_SI(_max, 'm', unit)
    add_filled_area(ax, _min.time.values, _min.values, _max.values, label = 'Range', kwargs = fill_between_kwargs)  
    ax.plot(_mean.time.values, _mean.values, c = c[0], label = 'Mean', **adjusted_plot_kwargs)    
    add_horizontal_line(ax, horizontal_line, unit = unit)
    if legend: ax.legend()        
...
def plot_overlap(Model, time = -1, cutoff = 0.01, 
                 buffer = 0,
                 reservoir = None,
                 plot_reservoir_shapes = True, 
    ...
    clabel_kwargs = set_defaults(clabel_kwargs, defaults = Model.clabel_defaults)
    colorbar_kwargs = set_defaults(colorbar_kwargs, defaults = Model.colorbar_defaults)
    overlap_kwargs = set_defaults(overlap_kwargs, defaults = Model.raster_defaults)
    annotation_kwargs = set_defaults(annotation_kwargs, defaults = Model.annotation_defaults)
    scatter_kwargs = set_defaults(scatter_kwargs, defaults = Model.scatter_defaults)
    
    if variable != 'subsidence':
        warn(f'Warning: Variable {variable} has no meaningful measure for overlap.')
    time_index = time_entry_to_index(Model, time)
    if len(time_index) != 1:
        warn('Warning: More than 1 timestep is requested, final timestep in sseries is displayed.')
    cutoff_units = _utils.convert_SI(cutoff, 'm', unit)
    if title is None: title = f"Overlap of {cutoff_units} {unit} {variable} areas between reservoirs"
    reservoir_index = reservoir_entry_to_index(Model, reservoir)
    data = ((Model[variable].isel(reservoir = reservoir_index) < -cutoff)
            .sum(dim = 'reservoir')
            .isel(time = time_index[-1]))
    raster = {'X': data.x.values, 'Y': data.y.values, 'mask': np.flip(data.values, axis = 0)}
    
    
    if not plot_subsidence_contours:
        fig, ax = plot_reservoirs(Model, reservoir = reservoir, model = None,
        ...
        fig, ax =  plot_subsidence_model(Model, reservoir = None, time = time_index[-1], buffer = buffer, 
                                          variable = variable,
                                          unit = unit,
                                          plot_reservoir_shapes = plot_reservoir_shapes, 
                                          additional_shapes = additional_shapes, 
    ...
    adjusted_overlap_kwargs = set_defaults({'zorder': 10000}, defaults = overlap_kwargs)
    ...
    if legend:
        ...
        for i, value in enumerate(values):
            label = f"Overlap between {values[i]} reservoirs" if i != 0 else 'No overlap'
        ...
        ax.legend(handles=patches, loc='center left', bbox_to_anchor=bbox_to_anchor)
...
def _individual_m(ax, Model, variable, unit, reservoir_index, line_dict, steps, num,
                c, 
                plot_kwargs = {}, annotation_kwargs = {}):
    c_index = 0
    for i, r in enumerate(Model.reservoirs):
        if i in reservoir_index:
            data = get_2D_data_from_model(Model, reservoir = i, time = steps, variable = variable, unit = unit)
            
            add_cross_section(ax, Model.x, Model.y, [data], line_dict.values(),
                              name = [r],
                              inflection_point_names = None, c = [c[c_index]],
                              num = num, 
                              plot_kwargs = plot_kwargs,
                              annotation_kwargs = annotation_kwargs)
    ...
    total_data = get_2D_data_from_model(Model, reservoir = reservoir_index, 
                                        time = steps, variable = variable, 
                                        unit = unit)
    add_cross_section(ax, Model.x, Model.y, [total_data], line_dict.values(),
                      name = ["Total"],
                      inflection_point_names = None, c = 'k',
                      num = num, 
                      plot_kwargs = plot_kwargs,
                      annotation_kwargs = annotation_kwargs) 
...
def _cumulative_m(ax, Model, variable, unit, reservoir_index, line_dict, steps, num,
                c, fill_between_kwargs = {}):
    _, adjusted_fill_between_kwargs = seperate_colors_from_dict(fill_between_kwargs, len(reservoir_index))
    c_index = 0
    areas = []
    for i, r in enumerate(reservoir_index):
        adjusted_fill_between_kwargs['facecolor'] = c[c_index]
        adjusted_fill_between_kwargs['zorder'] = -c_index # make sure the shape is below the previous
        data = get_2D_data_from_model(Model, 
                                      reservoir = reservoir_index[:i +1], 
                                      time = steps, 
                                      variable = variable, 
                                      unit = unit)
        (values, distances, 
         inflection_distances) = get_cross_section(Model.x, 
                                                   Model.y, 
                                                   [data], 
                                                   line_dict.values(), 
                                                   num)
        
        area = add_filled_area(ax, distances, np.zeros_like(distances), values[0], 
        ...
        for l in range(len(line_dict) - 1):
            if l == 0:
                ...
                ax.axvline(x = inflection_distances[0], c = 'k', linestyle = 'dashed', alpha = 0.25, label = '_nolegend_')
                ax.annotate(list(line_dict.keys())[l], 
                            (inflection_distances[0], 0), path_effects = WHITE_SHADOW)
            
            ax.axvline(x = inflection_distances[l + 1], c = 'k', linestyle = 'dashed', alpha = 0.25, label = '_nolegend_')
            ax.annotate(list(line_dict.keys())[l + 1], (inflection_distances[l + 1], 0), path_effects = WHITE_SHADOW)
