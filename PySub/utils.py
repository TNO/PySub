...
class letter_counter:
    ...
    def get_letter(self):
        return string.ascii_uppercase[self.count % 26]
...
def isjagged(arr):
    """Returns True if a numpy array is jagged and False if it is a regular
    numpy array. Raises exception if not a numpy array.
    """
...
def concavity_magnitude(values, dx, dy):
    ...
    magnitude = np.linalg.norm(
        np.array([ddu_dxx, ddu_dxy, ddu_dyx, ddu_dyy]), axis=0
    )
    return magnitude
...
def get_chunked(ds, chunk_dimensions=["time", "reservoir"]):
    ...
    if chunk_dimensions == "all":
        ...
        return ds.chunk(
            {dim: (1 if dim in chunk_dimensions else -1) for dim in ds.dims}
        )
...
def get_van_opstal_coefficients(v):
    ...
    try:
        assert all(
            (0 < v) & (v < 0.45)
        ), f"Poisson's ratio must be between 0 and 0.45. Current value: {v}."
    except:
        assert (
            0 < v < 0.45
        ), f"Poisson's ratio must be between 0 and 0.45. Current value: {v}."
    coefficient_directory_name = os.path.join(
        os.path.dirname(__file__), "coefficients", "van_opstal_terms.csv"
    )
    parameter_df = pd.read_csv(coefficient_directory_name, index_col=0)
    parameter_dict = dict(parameter_df)
    coefficients = {p: poly_func(v, *c) for p, c in parameter_dict.items()}
    return coefficients
...
def datetime_to_years_as_float(time):
    ...
    if is_list_of_datetime(time):
        return (
            (pd.to_datetime(time) - pd.to_datetime(time[0])).astype(int)
            * 3.16887646
            * 1e-17
        ).values
...
def interpolate_over_time(timesteps, values, times_to_interpolate):
    ...
    _times_to_interpolate = (
        pd.to_datetime(times_to_interpolate) - timesteps[0]
    ).astype(int)
    return np.interp(
        _times_to_interpolate,
        _timesteps,
...
def pad_series(
    timesteps, values, start_time, end_time, start_value, end_value
):
...
def profile(
    ...
    if (end_value >= start_value).any():
        raise Exception(
            "End value must be lower than start value. More complex pressure profiles than a start and end pressure should be entered manually in the template."
        )
    ...
    if not is_iterable(profile):
        raise Exception(
            "The profile must be a list, tuple or numpy array of numbers."
        )
    else:
    ...
    if len(profile.shape) == 1:
        profile = np.repeat(
            np.expand_dims(profile, 0), number_of_reservoirs, axis=0
        )
    elif len(profile.shape) == 2:
        if profile.shape[0] != number_of_reservoirs:
            raise Exception(
                f"The number of profiles ({profile.shape[0]}) does noet equal the number of reservoirs ({number_of_reservoirs})."
            )

    sub_profile = np.array(sub_profile)
    if len(sub_profile.shape) == 1:
        sub_profile = np.repeat(
            np.expand_dims(sub_profile, 0), number_of_reservoirs, axis=0
        )
    elif len(sub_profile.shape) == 2:
        if sub_profile.shape[0] != number_of_reservoirs:
            raise Exception(
                f"The number of sub profiles ({sub_profile.shape[0]}) does noet equal the number of reservoirs ({number_of_reservoirs})."
            )
    ...
    for i in range(number_of_reservoirs):
        ...
        if number_of_years < 2:
            raise Exception(
                "Too little time between start and time to establish a proper profile."
            )

        beta = min(1, (number_of_years - beta_min) / beta_max)

        interpolated_profile = np.array(
            beta * profile[i] + (1 - beta) * sub_profile[i]
        )
        transformed_profile = end_value[i] + interpolated_profile * (
            start_value[i] - end_value[i]
        )
        coords = np.array(list(range(len(transformed_profile))))
        f = interpolate.interp1d(
            coords, transformed_profile, fill_value="extrapolate"
        )
        location_time = np.where(
            (years <= end_time[i]) & (years >= start_time[i])
        )[0]
...
def interpolate_grid_from_points(points, values, XY, method="linear"):
    ...
    grid_z = interpolate.griddata(
        points=points, values=values, xi=(x, y), method=method, fill_value=0
    )
    return grid_z
...
def point_or_points(points):
    ...
    if not (x_are_numbers and y_are_numbers):
        raise Exception(
            "Point indicators can only be: an integer for indexing, floats for coordinates or a list of integers or floats."
        )
    return _points, x, y
...
def check_shapefiles(shapefile_paths):
    is_file = [
        os.path.isfile(shp) and shp.endswith(".shp") for shp in shapefile_paths
    ]
    check_file = np.where(np.logical_not(is_file))[0]
    if len(check_file) > 0:
        raise Exception(
            f"Invalid shapefiles: {np.array(shapefile_paths)[check_file]}"
        )
    ...
    if var is None:
        ...
        if is_list_of_strings(list(var)):
            ...
            if len(wrong) > 0:
                ...
                test = input(
                    f"Invalid input parameter '{var_name}': {_var[wrong]}. Are you sure? Type: y/n\n"
                )
                valid_input = False
...
def check_file_extension(path, extension=[".csv", ".txt"]):
    ...
    if is_iterable(extension):
        if not is_list_of_strings(extension):
            raise Exception(
                f"Invalid data type encountered in list of data extension type: {extension}. Expected strings."
            )
    ...
    if not file_extension:
        ...
        raise Exception(
            f"Unknown file extension {file_extension} detected, use {list_of_valid_extensions}. Use no extension to default to {list_of_valid_extensions[0]}."
        )
...
def _check_df_all_filled_or_all_nan(df):
    check = np.logical_not(
        np.logical_and(
            _check_df_any_nans(df), np.logical_not(_check_df_all_nan(df))
        )
    )
    ...
    if len(wrong_columns) > 0:
        raise Exception(
            f"The columns {wrong_columns} should either have all values filled or all values emtpy (in the case of optional values). Empty and filled values haven been encountered."
        )


def check_df(
    df, sheet_name, exclude=[], columns=[], must_haves=[], no_empty=True
):
    if len(columns) != 0:
    ...
    if df.shape[0] != 0:
        if no_empty:
            ...
            empty = np.unique(
                np.array(must_haves)[
                    np.where(df.isnull()[must_haves].values)[1]
                ]
            )
        missing = a_missing_from_b(empty, exclude)
        if len(missing) > 0:
            raise Exception(
                f'From the worksheet "{sheet_name}" the variable(s): {missing} are missing.'
            )
...
def export_df(df, path, extension=[".csv", ".txt"]):
    ...
    if not isinstance(path, str):
        raise Exception(
            f"File {path} must be a string with the path to the save file location."
        )
    else:
...
def isSubsidenceModel(Model):
    ...
    return (
        str(type(Model)).endswith(".SubsidenceModel'>")
        or str(type(Model)) == "<class 'PySub.MergedModel.MergedModel'>"
        or str(type(Model)) == "<class 'PySub.BucketEnsemble.BucketEnsemble'>"
    )
...
def check_Name_df(df):
    ...
    for i, r in enumerate(reservoir_names):
        ...
        reservoir_model_missing = a_missing_from_b(
            model_name, unique_model_names
        )
        if len(reservoir_model_missing) > 0:
            print(
                f"Warning: Model name(s) {reservoir_model_missing} have been defined, but miss from reservoir {r}"
            )
        model_missing_reservoir = a_missing_from_b(
            unique_model_names, model_name
        )
        if len(model_missing_reservoir) > 0:
            print(
                f"Warning: Reservoir {r} has no data defined for model(s) {model_missing_reservoir}"
            )
...
def check_Name(model_parameters, reservoir_parameters, pressure_df):
    ...
    """
    reservoir_names_reservoir, model_names_reservoirs = check_Name_df(
        reservoir_parameters
    )
    reservoirs_names_pressure, model_names_pressure = check_Name_df(
        pressure_df
    )
    model_names_model = model_parameters.index.values
    except_if_not_unique("Model name(s)", model_names_model)

    missing = check_if_all_uniques_are_same(
        model_names_reservoirs.values,
        model_names_pressure.values,
        model_names_model,
    )
    ...
    if len(missing) > 0:
        raise (
            Exception(
                f"The reservoirs {missing} are missing from one of the entered data sheets."
            )
        )
    ...
    for r in range(number_of_reservoirs):
        if model_names_reservoirs[r] != model_names_pressure[r]:
            raise (
                Exception(
                    f"The data for reservoir and pressure show different model names for reservoir {reservoir_names[r]}"
                )
            )
...
def reorder_pressure_to_reservoir_parameters(
    reservoir_parameters, pressure_df
):
    ...
    """
    reservoir_parameters["Name and Name type"] = (
        reservoir_parameters.index + reservoir_parameters["Name"]
    )
...
def check_if_unique(values):
    ...
    if not is_iterable(values):
        raise Exception(
            f"Cannot determine if the list {values} contains only unique values, for it is not iterable."
        )
    all_unique = True
...
def convert_to_datetime(timesteps):
    ...
    try:
        ...
        raise Exception(
            "Timesteps should be a np.ndarray with entries in integers for years, np.datetime64 or a pd.Timestamp object."
        )
    ...
    if np.issubdtype(timesteps.dtype, np.integer):
        ...
        try:
            ...
            raise Exception(
                "The date or ont of the dates entered is too early (or late) for us computers to understand. Consider entering time as an integer to consider times before 1700."
            )
        except:
            raise Exception(
                "Timesteps should be a np.ndarray with entries in integers for years or np.datetime64 or a pd.Timestamp object."
            )
    return timesteps
...
def bounds_from_bounds_collection(bound_collection):
    lowest_x, highest_x = np.min(bound_collection[:, 0]), np.max(
        bound_collection[:, 2]
    )
    lowest_y, highest_y = np.min(bound_collection[:, 1]), np.max(
        bound_collection[:, 3]
    )
    return [lowest_x, lowest_y, highest_x, highest_y]
...
def moving_rigid_basement(
    start_basement, end_basement, reservoir_depth, time, viscous_tau
):
    ...
    moving_basement = (
        reservoir_depth / start_basement
