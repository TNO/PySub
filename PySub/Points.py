...
class PointCollection:
    ...
    def __init__(self, list_of_Points):
        ...
        if not (type_entry is list or type_entry is np.ndarray):
            if isinstance(list_of_Points, Point):
                ...
                raise Exception(
                    "Entries should be a list of Point objects or a single Point object."
                )
        types = np.array([isinstance(p, Point) for p in list_of_Points])
        if np.logical_not(types).any():
            raise Exception(
                "Entries should be a list of ObservationPoint objects or a single ObservationPoint."
            )
        ...
        if isinstance(item, int) or isinstance(item, slice):
            ...
            if item in self.names:
                return self.collection[
                    np.where(np.array(self.names) == item)[0][0]
                ]
            else:
    ...
    def as_df(self):
        ...
        """
        df = pd.DataFrame(
            self.coordinates, index=self.names, columns=["X", "Y"]
        )
...
class ObservationPoint:
    ...
    def check_length(self, var, name):
        if not _utils.is_iterable(var):
            raise Exception(
                "Input must be a list, numpy array or pandas Series object."
            )
        var = np.array(var)
        if len(var.shape) != 1:
        ...
        if self.hasattr("number_of_entries"):
            if self._number_of_entries != len(var):
                raise Exception(
                    f"Number of entries, set by {self._variable_that_set_number_of_entries}, do not match with length of {name}. No. of entries: {self._number_of_entries}"
                )
        else:
...
class ObservationCollection:
    ...
    def __init__(self, list_of_ObservationPoints):
        ...
        types = np.array(
            [
                isinstance(p, ObservationPoint)
                for p in list_of_ObservationPoints
            ]
        )
        if np.logical_not(types).any():
            raise Exception(
        ...
        self._number_of_entries = [
            p.number_of_entries for p in self.collection
        ]
    ...
    def as_df(self):
        ...
        """
        IDs = [
            [o.name for _ in self.observations[i]]
            for i, o in enumerate(self.collection)
        ]
        ...
        X = [
            [o.x for _ in self.observations[i]]
            for i, o in enumerate(self.collection)
        ]
        Y = [
            [o.y for _ in self.observations[i]]
            for i, o in enumerate(self.collection)
        ]
        ...
        try:
            df["Lower error (m)"] = _utils.flatten_ragged_lists2D(
                self.lower_errors
            )
        except:
            pass
        try:
            df["Upper error (m)"] = _utils.flatten_ragged_lists2D(
                self.upper_errors
            )
        except:
            pass
        return df.set_index("Observation ID")

    @property
    def most_subsidence(self):
        last_value_of_each_observations_point = [i[-1] for i in self.relative]
        observation_point_with_most_observed_subsidence = np.where(
            last_value_of_each_observations_point
            == np.max(last_value_of_each_observations_point)
        )[0][0]
        i = int(observation_point_with_most_observed_subsidence)
        return self[i]


def load_points_from_df(point_df, x_column="X", y_column="Y"):
    ...
    for val in check_columns:
        if not _utils.is_str_or_int(val):
            raise Exception(
                f"Invalid input {val}, enter string for column name or integer for column index"
            )

    missing_columns = _utils.a_missing_from_b(
        check_columns, point_df.columns.values
    )
    if len(missing_columns) > 0:
        raise Exception(
            f"The columns {missing_columns} are not in the Excel sheet."
        )
...
def load_observation_points_from_df(
    observation_df,
    ...
    missing_columns = _utils.a_missing_from_b(
        check_columns, observation_df.columns.values
    )
    if len(missing_columns) > 0:
        raise Exception(
            f"The columns {missing_columns} are not in the Excel sheet."
    ...
    for observation, name in enumerate(observation_unique_ids):
        ...
        if len(x) != 1 and len(y) != 1:
            raise Exception(
                "Observations cannot change location over time, check x and y cordinates and secure uniform x- and y-coordinates."
            )
        x, y = x[0], y[0]
        ...
        observation_point.set_parameters(time, obs, le, ue)
