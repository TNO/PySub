...
class SubsidenceModel:
    ...
    def __str__(self):
        ...
        build_df.loc["number_of_reservoirs"] = (
            self.hasattr("number_of_reservoirs"),
            "set_reservoirs",
        )
        build_df.loc["number_of_steps"] = (
            self.hasattr("number_of_steps"),
            "set_timesteps",
        )
        ...
        point_df.loc["number_of_points"] = (
            self.hasattr("number_of_points"),
            "set_points",
        )
        point_df.loc["point_subsidence"] = (
            self.hasattr("point_subsidence"),
            "calculate_subsidence_at_points",
        )

        observation_df = pd.DataFrame(columns=columns)
        observation_df.loc["observation_points"] = (
            self.hasattr("observation_points"),
            "set_obersvation_points",
        )
        observation_df.loc["number_of_observation_points"] = (
            self.hasattr("number_of_observation_points"),
            "set_obersvation_points",
        )
        observation_df.loc["observation_subsidence"] = (
            self.hasattr("observation_subsidence"),
            "calculate_subsidence_at_observations",
        )
    ...
    def compaction(self):
        ...
        try:
            ...
            raise Exception("Compaction has not been set/calculated yet.")
    ...
    def subsidence(self):
        ...
        try:
            ...
            raise Exception("Subsidence has not been calculated yet.")
    ...
    def slope(self):
        ...
        try:
            ...
            raise Exception(
                "Calculate susbsidence before calculating subsidence gradient."
            )
    ...
    def subsidence_rate(self):
        ...
        try:
            ...
            raise Exception("Volume has not been calculated yet.")
    ...
    def concavity(self):
        ...
        """
        try:
            return self.grid.concavity_x
        except:
            raise Exception("Concavity has not been calculated yet.")
    ...
    def point_subsidence(self):
        ...
        try:
            ...
            raise Exception("Point subsidence has not been calculated yet.")
    ...
    def observation_subsidence(self):
        ...
        try:
            ...
            raise Exception(
                "Observation subsidence has not been calculated yet."
            )
    ...
    def total_subsidence(self):
        ...
        """
        try:
            return self.grid.subsidence.sum(dim="reservoir")
        except:
            raise Exception(
                "Calculate susbsidence before calculating subsidence total."
            )
    ...
    def _check_projection(self, projection, file_name):
        if projection != self._projection:
            raise Exception(
                f"Two different projections have been encountered: {projection} and {self.projection}."
            )
        ...
        if dims_equal == "reservoirs":
            if self.number_of_reservoirs is None:
                ...
                self._dim_exception(
                    ...
                    dims_equal,
                    self.number_of_reservoirs,
                    self._variable_that_set_number_of_reservoirs,
                )
            ...
            if self.number_of_steps is None:
                ...
                self._dim_exception(
                    ...
                    dims_equal,
                    self.number_of_steps,
                    self._variable_that_set_number_of_steps,
                )
            ...
            if self.number_of_points is None:
                ...
                self._dim_exception(
                    ...
                    self._variable_that_set_number_of_points,
                )
        elif dims_equal == "x":
            if len(var) != self.nx:
                self._dim_exception(
    ...
    def _check_for_model_attributes(self, var, action="perform this action"):
        if not isinstance(var, str):
            raise Exception(
                f"When checking attribute existance, attribute name {var} needs to be a string"
            )
        if not self.hasattr(var):
            raise Exception(
                f"SubsidenceModel object has no attribute {var}, this is required to {action} of the model. Use the method SubsidenceModel.set_{var} to proceed."
            )
    ...
    def hasattr(
        self, var
    ):  # redefenition for convenient syntax and lack of editable dunder method
    ...
    def _check_crs(self, crs):
        if not self.hasattr("crs"):
            ...
            if crs != self._crs:
                raise Exception(
                    f"Sources use different coordinate reference systems: {crs} and {self._crs}"
                )
    ...
    def check_subsidence_model(self, name_model):
        ...
        if isinstance(name_model, str):
            if name_model.lower() not in ["knothe", "nucleus of strain"]:
                raise Exception(
                    f"{name_model} not recognized as valid subsidence model. Choose from: 'knothe', 'nucleus of strain''"
                )
        else:
            raise Exception(
                "set the subsidence model using a string with either 'knothe', 'nucleus of strain'"
            )
    ...
    def check_for_grid(self):
        ...
        if not self.built:
            raise Exception(
                f"No grid has been build for this model: {self.name}."
            )
    ...
    def check_influence_radius(self):
        ...
        if not hasattr(self.grid, "reservoir_mask"):
            raise Exception(
                "Reservoir mask layer not set, run mask_from_shapefiles before checking influence radius"
            )
        for reservoir in range(self.number_of_reservoirs):
            lx, ly, hx, hy = self.get_id_bounds_reservoir(
                reservoir_layer=reservoir
            )
            if (hx - lx) // 2 > self._influence_radius // self._dx or (
                hy - ly
            ) // 2 > self._influence_radius // self._dy:
                if (hx - lx) // 2 >= (hy - ly) // 2:
                    max_distance = np.round(((hx - lx) // 2) * self._dx)
                else:
                    max_distance = np.round(((hy - ly) // 2) * self._dy)
                warn(
                    f"The influence radius {self._influence_radius} less than half of the largest reservoir: {max_distance}."
                )
    ...
    def add_kernels(self):
        ...
        """
        self._kernels = [
            _SubsidenceKernel.InfluenceKernel(self._influence_radius, self._dx)
    ...
    def add_point_kernels(self, points=None):
        ...
        point_kernels = []
        for point in points:
            point_kernels_per_point = [
                _SubsidencePointKernel.InfluencePoint(
                    self.grid, point, self._influence_radius
                )
                for r in range(self.number_of_reservoirs)
            ]
            for reservoir in range(self.number_of_reservoirs):
                if self._subsidence_model_type.lower().startswith("nucleus"):
                    point_kernels_per_point[reservoir].nucleus(
                        self._depths[reservoir],
                        self._depth_to_basements[reservoir],
                    )
                elif self._subsidence_model_type == "knothe":
                    point_kernels_per_point[reservoir].knothe(
                        self._depths[reservoir], self.knothe_angles[reservoir]
                    )
    ...
    def assign_data_variable(self, name, dimensions, var):
        ...
        if var is not None:
            ...
            if hasattr(self.grid, "reservoir_mask") and "x" in dimensions:
                self.grid[str(name)] = xr.where(
                    self.grid["reservoir_mask"], self.grid[str(name)], 0
                )
    ...
    def assign_observation_parameters(self):
        ...
        if self.hasattr("observation_points"):
            self.assign_new_coordinates(
                "observations", self.observation_points.names
            )
    ...
    def assign_1D_or_2D(self, var, name):
        ...
        if len(var_shape) == 2:
            try:
                self.assign_data_variable(name, ("reservoir", "time"), var)
            except:
                self.assign_data_variable(name, ("y", "x"), var)
        if len(var_shape) == 1:
    ...
    def get_contour_levels(
        ...
        if levels is not None:
            if not _utils.is_iterable(levels):
                raise Exception(
                    f"Invalid type ({type(levels)}) for setting contour levels. Use list."
                )
        elif (
            start is not None and end is not None and contour_steps is not None
        ):
            ...
            if "reservoir" in list(as_array.coords):
                min_value, max_value = (
                    np.min(
                        (
                            np.min(self[variable].sum(dim="reservoir").values),
                            -contour_steps,
                        )
                    ),
                    np.max(
                        (
                            np.max(self[variable].sum(dim="reservoir").values),
                            contour_steps,
                        )
                    ),
                )
                ...
                min_value, max_value = (
                    np.min((np.min(self[variable].values), -contour_steps)),
    ...
    def get_colors_grouped(
        self, cmap="brg", argument={"similar_first": 3}, group_colors=None
    ):
        ...
        if isinstance(argument, dict):
            ...
            if argument_type == "similar_first":
                if not isinstance(argument_value, int):
                    raise Exception(
                        'The argument value for the argument type "similar_first" must be an integer, which indicates the number of letters in the text that should be the same from the start.'
                    )
                sort_by = [
                    occurence[:argument_value] for occurence in self.reservoirs
                ]
            elif argument_type == "similar_end":
                if not isinstance(argument_value, int):
                    raise Exception(
                        'The argument value for the argument type "similar_end" must be an integer, which indicates the last few number of letters in the text that should be the same from the start.'
                    )
                sort_by = [
                    occurence[:argument_value] for occurence in self.reservoirs
                ]
                ...
                if not isinstance(argument_value, list):
                    raise Exception(
                        'The argument value for the argument type "similar_name" must be a list, where each entry is a segement of text present in the isolated groups.'
                    )
                sort_by = []
                for occurence in self.reservoirs:
                    corresponding_values = [
                        v in occurence for v in argument_value
                    ]
                    if any(corresponding_values):
                        values_that_contain_that_segment = np.array(
                            argument_value
                        )[corresponding_values]
                        longest_value_index = np.argmax(
                            values_that_contain_that_segment
                        )
                        sort_by.append(
                            values_that_contain_that_segment[
                                longest_value_index
            ...
            raise Exception(
                "Additional arguments should be of a dict type with the shape: dict(argument_type = argument_value)"
            )

        return _plot_utils.get_colors_grouped(
            self.reservoirs, sort_by, cmap=cmap, group_colors=group_colors
        )
    ...
    def set_points(self, points):
        ...
        if not isinstance(points, _Points.PointCollection):
            if isinstance(points, _Points.Point):
                ...
                raise Exception(
                    "Points set into model need to be PointCollection or Points objects from the Point module"
                )
    ...
    def set_observation_points(self, observation_points):
        ...
        if _utils.is_iterable(observation_points):
            if (
                ...
                for i in observation_points:
                    if (
                        not str(type(i))
                        == "<class 'PySub.Points.ObservationPoint'>"
                    ):
                        raise Exception(
                            "Entries should be a list or numpy array with ObservationPoint objects from the Points module."
                        )
                observation_points = _Points.ObservationCollection(
                    observation_points
                )
        else:
            raise Exception(
        ...
        self.number_of_observation_points = (
            self._observation_points.number_of_observation_points
        )
    ...
    def load_from_raster(self, fname, layer=None):
        ...
        if not (
            self.hasattr("number_of_steps")
            and self.hasattr("number_of_reservoirs")
            and self.built
        ):
            raise Exception(
                "To set variable from a raster file, built the grid first."
            )
        reshaped_data = np.transpose(loaded_data, axes=(1, 2, 0))
        data_xr = xr.DataArray(
            reshaped_data,
            (("y", y), ("x", x), ("band", np.arange(number_of_bands))),
        )
        interpolated_data = data_xr.interp_like(
            self.grid, method="nearest", kwargs={"fill_value": 0}
        )
        return interpolated_data.values

    def load_from_csv(self, fname, delimiter=";", header=0, decimal=","):
        loaded_data, x, y = _shape_utils.load_raster_from_csv(
        ...
        data_xr = xr.DataArray(
            ...
            (
                ("y", y[:, 0]),
                ("x", x[0, :]),
                ("band", np.arange(number_of_bands)),
            ),
        )
        interpolated_data = data_xr.interp_like(
            self.grid, method="nearest", kwargs={"fill_value": 0}
            ...
            if isinstance(var, str):
                ...
                if var.endswith("tif"):
                    ...
                    loaded_var = self.load_from_csv(
                        var, delimiter=";", header=0
                    )
                else:
                    raise Exception(
                        "Invalid file type encountered. Supported file types: .tif, .txt, .csv."
                    )
                number_of_bands = loaded_var.shape[-1]
                if number_of_bands != self.number_of_reservoirs:
                    raise Exception(
                        f"Number of bands {number_of_bands} disequals number of reservoirs: {self.number_of_reservoirs}."
                    )
            ...
            if _utils.is_list_of_strings(list(var)):
                ...
                for path in list_of_paths:
                    ...
                    if number_of_bands == 1:
                        ...
                        raise Exception(
                            f"Invalid number of bands in .tif file: {number_of_bands}. Enter as a string for a .tif file with all the reservoirs in it, as a list for tif files with each one reservoir."
                        )
                ...
                raise Exception(
                    f"Invalid type {type(var)} for setting variable."
                )
    ...
    def set_shapes(self, shapes):
        ...
        if shapes is not None:
            if not _utils.is_iterable(shapes):
                raise Exception(
                    "Set shapes with lists of the available datatypes (str for .shp, .csv or .tif files or xy-coordinates.)"
                )
            self._check_dim1D("shapes", shapes)
            self._shapes = _Geometries.fetch(
                shapes,
    ...
    def set_poissons_ratios(self, poissons_ratios):
        if _utils.isnan(np.array(poissons_ratios)) or poissons_ratios is None:
            raise Exception(
                f"Poisson's ratio must be entered. Current values {poissons_ratios}"
            )
        if not poissons_ratios is None:
            _utils._check_low_high(
                poissons_ratios, "poisson_ratios", EPSILON, 0.45
            )
    ...
    def set_timesteps(self, timesteps):
        ...
        if self.number_of_steps < 2:
            raise Exception(
                f"Not enough timesteps to calculate compaction/subsidence. At least 2 timesteps are required. \n Current number of timesteps: {self.number_of_steps}"
            )
    ...
    def set_bounds(self, bounds=None):
        ...
        if bounds is None:
            if self.shapes is not None:
                ...
                raise Exception(
                    "The function set_bounds requires 4 numeric entries or requires SubsidenceModel object to have shape- or gridfiles set."
                )
        else:
            ...
            if len(bounds) == 4:
                if self.hasattr("shapes"):
                    ...
                    if (
                        bounds[0] > np.min(shapes_per[:, 0])
                        or bounds[1] > np.min(shapes_per[:, 1])
                        or bounds[2] < np.min(shapes_per[:, 2])
                        or bounds[3] < np.max(shapes_per[:, 3])
                    ):
                        raise Exception(
                            f"Bounds {bounds} do not overlap with set shapes."
                        )
                self._bounds = bounds
    ...
    def set_dy(self, dy=None):
        ...
        if dy is None:
            if self._dx != None:
                ...
                raise Exception(
                    "Not able to set dy with None. Set dx or enter numerical value above 0."
                )
        else:
    ...
    def set_influence_radius(self, influence_radius):
        ...
        """
        _utils._check_low_high(
            influence_radius, "influence radius", -EPSILON, 100000
        )
    ...
    def set_knothe_angles(self, knothe_angles, layer=None):
        ...
        if knothe_angles is not None:
            ...
            if len(invalid_values) > 0:
                raise Exception(
                    f"Invalid values for knothe angles encountered: {knothe_angles[invalid_values]}"
                )
    ...
    def set_subsidence_model_parameters(
        ...
        if model == "knothe":
            if knothe_angles is not None:
                ...
                raise Exception(
                    "Knothe angles need to be defined for each reservoir when subsidence model type is Knothe."
                )
        ...
        if model == "nucleus of strain":
            if poissons_ratios is not None:
                ...
                if depth_to_basements is None:
                    self.set_depth_to_basements(
                        [None for _ in self.reservoirs]
                    )
                else:
                    self.set_depth_to_basements(depth_to_basements)
            else:
                raise Exception(
                    """Poisson's ratios' need to be defined for each reservoir when subsidence model type is "nucleus of strain"."""
                )
    ...
    def set_compaction(self, compaction):
        ...
        if not _utils.is_iterable(compaction):
            raise Exception(
                f"Compaction must be an iterable with the same shape as the model: {self.shape}. wrong type: {type(compaction)}."
            )
        ...
        if not compaction.shape == self.shape:
            raise Exception(
                f"Compaction must be an iterable with the same shape as the model: {self.shape}, {list(self.grid.coords)}. Wrong shape: {compaction.shape}."
            )

        self.assign_data_variable(
            "compaction", ["y", "x", "reservoir", "time"], compaction
        )
    ...
    def get_id_bounds_reservoir(self, reservoir_layer, grid=None):
        ...
        """
        ys, xs = np.where(
            self.grid.reservoir_mask.isel(reservoir=reservoir_layer) == 1
        )
        if len(xs) == 0:
            raise Exception(
                f"Reservoir {self.reservoirs[reservoir_layer]} extend not in grid!"
            )
    ...
    def mask_reservoirs(self):
        ...
        self.check_for_grid()
        model_grid = np.zeros(
            shape=(self.ny, self.nx, self.number_of_reservoirs)
        )
        for i, reservoir in enumerate(self._shapes):
            model_grid[:, :, i] = reservoir.mask(self.grid)
        self.assign_data_variable(
            "reservoir_mask", ("y", "x", "reservoir"), model_grid
        )
    ...
    def time_label_to_int(self, val):
        ...
        if self.timesteps.dtype == np.int64:
            if isinstance(val, str):
                try:
                    if int(val) in self.grid.time.values:
                        index = np.where(self.grid.time.values == int(val))[0][
                            0
                        ]
                except:
                    raise Exception(
                        f"The key {val} is not an available timestep"
                    )
            elif isinstance(val, int):
            ...
            if isinstance(val, int):
                times_as_years = np.datetime_as_string(
                    self.timesteps, unit="Y"
                ).astype(int)
                ...
                times_yy_mm_dd = np.datetime_as_string(
                    self.timesteps, unit="D"
                )
                times_as_years = np.datetime_as_string(
                    self.timesteps, unit="Y"
                )
    ...
    def observation_label_to_int(self, val):
        ...
        if isinstance(val, str):
            if str(val) in self.grid.observations.values:
                index = np.where(self.grid.observations.values == str(val))[0][
                    0
                ]
            else:
                raise Exception(
                    f"The key {val} is not an available observation point"
                )
        elif isinstance(val, int):
            index = val
        else:
    ...
    def get_subsidence_with_ids(self, reservoir=None, time=None):
        ...
        if reservoir is None:
            ...
            if time is None:
                data = np.zeros((self.ny, self.nx, self.number_of_steps))
                for step in range(self.number_of_steps):
                    data[:, :, step] = self.subsidence.isel(
                        reservoir=reservoir, time=step
                    )
            else:
                data = np.zeros((self.ny, self.nx, 1))
                time_i = self.time_label_to_int(time)
                data[:, :, 0] = self.subsidence.isel(
                    reservoir=reservoir, time=time_i
                )
        return data

    def get_timeseries_coords(
        self, variable="subsidence", reservoir=None, x=None, y=None
    ):
        if not hasattr(self, variable):
            raise Exception(
                f"SubsidenceModel {self.name} has no attribute {variable}."
            )
        reservoir_index = _plot_utils.reservoir_entry_to_index(self, reservoir)

        variable_values = (
            getattr(self, variable)
            .isel(reservoir=reservoir_index)
            .sum(dim="reservoir")
        )
    ...
    def get_timeseries(self, x, y, variable="subsidence", reservoir=None):
        ...
        if not self.bounds[0] < x < self.bounds[2]:
            warn(
                f"X-coordinate {x} does not fall within the range of x-coordinates of the model ({self.bounds[0]}, {self.bounds[2]})."
            )
        if not self.bounds[1] < y < self.bounds[3]:
            warn(
                f"Y-coordinate {y} does not fall within the range of y-coordinates of the model ({self.bounds[1]}, {self.bounds[3]})."
            )

        timeseries = self.get_timeseries_coords(
            variable=variable, reservoir=reservoir, x=x, y=y
        )
        return timeseries
    ...
    def get_subsidence_overview(self, group=None):
        ...
        if group is not None:
            if not all([isinstance(subgroup, list) for subgroup in group]):
                raise Exception(
                    "The format for groups should be a list of lists representing groups."
                )
            flattened_grouping = [j for sub in group for j in sub]
            if len(flattened_grouping) > self.number_of_reservoirs:
                raise Exception(
                    "Too many reservoirs. Grouping needs to be some, or all of the reservoirs."
                )
            filtered_group = []
            for subgroup in group:
                filtered_group.append(
                    [r for r in subgroup if r in self.reservoirs]
                )
            check_if_unique, doubles = _utils.check_if_unique(
                flattened_grouping
            )
            if not check_if_unique:
                raise Exception(
                    f"The reservoirs: {doubles} are referenced more then once, which conflicts with the grouping."
                )

        subsidence_dict = {}
        subsidence_at_deepest_point = {
            r: self.get_max_subsidence(reservoir=r)[0] for r in self.reservoirs
        }
        subsidence_dict[self.name] = subsidence_at_deepest_point

        subsidence_df = pd.DataFrame(subsidence_dict)
        subsidence_df = pd.concat(
            [
                subsidence_df,
                pd.DataFrame(
                    [[self.get_max_subsidence()[0]]],
                    index=["Total"],
                    columns=[self.name],
                ),
            ]
        ...
        if group is not None:
            for subgroup in filtered_group:
                group_subsidence = self.get_max_subsidence(reservoir=subgroup)[
                    0
                ]
    ...
    def get_max_subsidence_timeseries(self, reservoir=None):
        ...
        timeseries = self.get_timeseries(x, y, reservoir=reservoir)
        return timeseries
    ...
    def build_grid(self):
        ...
        self.grid = generate_grid_from_bounds(
            self._bounds,
            self._dx,
            self._dy,
    ...
    def build_like(self, grid):
        ...
        if all(
            hasattr(grid, attr)
            for attr in ["bounds", "dx", "dy", "influence_radius"]
        ):
            self.grid = generate_grid_from_bounds(
                grid.bounds,
                grid.dx,
            ...
            warn(
                "Warning: grid has not enough information to build grid from."
            )

    def build_from_bounds(self, bounds, dx, dy=None, influence_radius=0):
    ...
    def calculate_subsidence(self, _print=True):
        ...
        if not hasattr(self.grid, "reservoir_mask"):
            raise Exception(
                "Reservoir mask not set, run mask_reservoirs before calculating"
            )
        if not self.hasattr("compaction"):
            raise Exception(
                "No compaction has been set/calculated, determine compaction before calculating subsidence."
            )
        if not self.hasattr("depths"):
            raise Exception(
                "Reservoir depth has not been set. Use set_depth before calculating subsidence."
            )

        self.check_influence_radius()

        # self.add_kernels()
        self.kernel = _SubsidenceKernel.InfluenceKernel(
            self._influence_radius, self._dx
        )

        if self._subsidence_model_type.lower().startswith("nucleus"):
            self.kernel.nucleus(
                self.depths, self.depth_to_basements, v=self.poissons_ratios
            )
    ...
    def calculate_subsidence_at_points(
        self, points=None, interpolate=True, _print=True
    ):
        ...
        self.assign_point_parameters()
        if _print:
            print(f"Calculating subsidence at points, model: {self.name}")

        if not self.hasattr("points") and points is None:
            warn(
                "Warning: No point objects have been defined in the model, or set as function parameters. No subsidence at points has been calculated."
            )
            return
        if interpolate and not self.hasattr("subsidence"):
            interpolate = False
            warn(
                "Warning: No subsidence has been calculated to interpolate over, calculating subsidence for points."
            )
        ...
        if interpolate:
            if points is None:
                ...
                try:
                    ...
                    raise Exception(
                        f"Invalid type of points: {type(points)}. Use an iterable with shape mx2, where m is the amount of points and 2 the x- and y-coordinate."
                    )
                if len(interpolate_points.shape) == 1:
                    interpolate_points = interpolate_points[None, :]
                if (
                    len(interpolate_points.shape) != 2
                    or interpolate_points.shape[1] != 2
                ):
                    raise Exception(
                        "Invalid indication of points. Use an iterable with shape mx2, where m is the amount of points and 2 the x- and y-coordinate."
                    )
            else:
                warn(
                    "Warning: No point objects have been defined in the model, or set as function parameters."
                )
                return
            x, y = zip(*interpolate_points)
            point_subsidence = -self.grid["subsidence"].interp(
                x=("z", np.array(x)), y=("z", np.array(y))
            )
        else:
            # Not nice code, prone to errors
            if not self.hasattr("compaction"):
                raise Exception(
                    "No compaction has been calculated, determine compaction before calculating subsidence."
                )
            if not hasattr(self.grid, "reservoir_mask"):
                raise Exception(
                    "Reservoir mask not set, run mask_reservoirs before calculating"
            ...
            for i in range(number_of_points):
                ...
                for reservoir in range(self.number_of_reservoirs):
                    ...
                    subsidence[:, :, reservoir, :] = s.transpose(
                        "y", "x", "time"
                    )
                point_subsidence[i, :, :] += subsidence.sum(axis=(0, 1))

        if _print:
            print(f"Calculated subsidence at points, model: {self.name}")

        if points is None:
            self.grid["point_subsidence"] = -point_subsidence.rename(
                {"z": "points"}
            )
            return self.grid["point_subsidence"]
        else:
    ...
    def calculate_subsidence_at_observations(
        self, interpolate=True, _print=True
    ):
        ...
        self.assign_observation_parameters()
        if _print:
            print(f"Calculating subsidence at observation, model: {self.name}")

        if not hasattr(self.grid, "reservoir_mask"):
            raise Exception(
                "Reservoir reservoir layer not set, run mask_from_shapefiles before calculating"
            )
        if not self.hasattr("compaction"):
            raise Exception(
                "No compaction has been calculated, determine compaction before calculating subsidence."
        ...
        if not self.hasattr("observation_points"):
            warn(
                "Warning: No observation points have been set in this model. Subsidence at the location of observations have not been calculated."
            )
            return
        points = self.observation_points.coordinates
        point_subsidence = self.calculate_subsidence_at_points(
            points=points, interpolate=interpolate, _print=False
        )
        point_subsidence = point_subsidence.rename({"z": "observations"})
        self.grid["observation_subsidence"] = -point_subsidence
        if _print:
            print(f"Calculated subsidence at observation, model: {self.name}")
        return self.grid["observation_subsidence"]
    ...
    def compare_observations(self):
        ...
        if self.hasattr("observation_points"):
            ...
            unique_times = np.unique(
                _utils.flatten_ragged_lists2D(self.observation_points.time)
            )
            interpolated_over_time = self.grid[
                "observation_subsidence"
            ].interp(time=unique_times)
            for p in self.observation_points:
                model_value = (
                    interpolated_over_time.sel(
                        time=p.time, observations=p.name
                    )
                    .sum("reservoir")
                    .values
                )
    ...
    def calculate_slope(self, reservoir=None, numeric=False, _print=True):
        ...
        if numeric:
            if not self.hasattr("subsidence"):
                raise Exception(
                    f"No subsidence has been designed for ModelSubsidence object {self.name}. Run calculate_subsidence and try again."
                )
            ...
            _reservoir = _plot_utils.reservoir_entry_to_index(self, reservoir)
            self.kernel = _HKernel.InfluenceKernel(
                self._influence_radius, self._dx
            )
            self.kernel.nucleus_slope(
                self.depths, self.depth_to_basements, self.poissons_ratios
            )
            self.grid["slope_x"] = (
                -xr.apply_ufunc(
                    _utils.convolve,
    ...
    def calculate_concavity(self, reservoir=None, numeric=False, _print=True):
        ...
        """
        if _print:
            print(f"Calculating subsidence concavity, model: {self.name}")
        _reservoir = _plot_utils.reservoir_entry_to_index(self, reservoir)
        if numeric:
            if not self.hasattr("subsidence"):
                raise Exception(
        ...
        if _print:
            print(f"Calculated subsidence concavity, model: {self.name}")
    ...
    def calculate_subsidence_rate(self, _print=True):
        ...
        """
        if _print:
            print(f"Calculating subsidence rate, model: {self.name}")
        ...
        timesteps_in_seconds = xr.DataArray(
            self.timesteps.astype(int) / 1e9,
        ...
        seconds_in_steps = xr.DataArray(
            np.diff(timesteps_in_seconds, prepend=timesteps_in_seconds[0]),
            coords={"time": self.timesteps},
        )

        self.grid["subsidence_rate"] = (
            du__dr_per_second * seconds_in_steps
        ).compute()
        if _print:
            print(f"Calculated subsidence rate, model: {self.name}")
        return self.grid["subsidence_rate"]
