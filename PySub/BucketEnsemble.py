    ...
    def check_probabilities(self, additional_message=""):
        ...
        for key, item in self.items():
            ...
            if (np.array(probabilities) < 0).any() or (
                ...
                raise Exception(
                    f"{additional_message}     The sum of the probabilities for the parameter {key} contain invalid values.\n{probabilities}"
                )
...
class BucketEnsemble(_SubsidenceModelGas.SubsidenceModel):
    ...
    def set_buckets(self, buckets, bounds=None):
        ...
        for r, bucket in buckets.items():
            ...
            self.write_lookup_table(file_loc, bucket)
    ...
    def write_lookup_table(self, file_loc, buckets):
        values, probabilities = buckets.as_list()
        max_length = max(len(entry) for entry in values)
        options = np.arange(max_length) + 1
        value_dict = {var: val for var, val in zip(buckets.variables, values)}
    ...
    def sample_from_buckets(self, seed=None):
        ...
        for i, var in enumerate(_memory.MODEL_VARIABLES):
            if var != "pressures":
                model_vars_dict[var] = (
                    model_vars[i]
                    if not (np.array(model_vars[i]) == None).any()
                    else None
                )
            else:
        ...
        if self.project_folder.project_folder is None:
            ...
            for var in read_columns:
                if var == "Pressure profile":
                    indices = {
                        reservoir: selected["Pressure profile"][reservoir]
                    ...
                    indices = {
                        reservoir: selected["Shapefile location"][reservoir]
                        for reservoir in self.reservoirs
                    }
                    parameters["shapes"] = [
                        self.buckets[reservoir]["shapes"]["Values"][index - 1]
                        for reservoir, index in indices.items()
                    ]
                else:
                    translated = _memory.COLUMN_TRANSLATOR[var]
                    ...
                    if selected.shape[0] == 1:
                        ...
                        parameters[translated] = [
                            selected[var][r] for r in self.reservoirs
                        ]
            sampled = {
    ...
    def move_through_list(self, i, l):
        ...
        if i < amount_of_samples:
            ...
            raise Exception(
                f"The sampled value {i} is not in the list with length {amount_of_samples}."
            )

    def get_all_deterministic_options(
        self,
    ):  # Not used in process, just for checking
        amount_of_options = {
            key: [len(var) if len(var) != 0 else 0 for var in reservoir.values]
    ...
    def sample_from_deterministic(self, i):
        ...
        for r, reservoir_name in enumerate(self.reservoirs):
            ...
            if r == 0:
                choises[reservoir_name] = self.move_through_list(
                    i % self.amount_of_runs_per_reservoir[reservoir_name],
                    self.amount_of_options[reservoir_name],
                )
            else:
                forked_possibilities = _utils.non_zero_prod(
                    list(self.amount_of_runs_per_reservoir.values())[:r]
                )
                choises[reservoir_name] = self.move_through_list(
                    np.floor(i / forked_possibilities)
                    % self.amount_of_runs_per_reservoir[reservoir_name],
                    self.amount_of_options[reservoir_name],
                )

            bucket = self.buckets[reservoir_name]
            probabilities[reservoir_name] = bucket.fetch(
                choises[reservoir_name], to_fetch="probabilities"
            )
    ...
    def calculate_deterministic(
        self, iterations=None, error_method="mae", all_timesteps=False
    ):
        ...
        self.amount_of_runs_per_reservoir = {
            key: _utils.non_zero_prod(self.amount_of_options[key])
    ...
    def calculate_samples(
        ...
        self.parameter_file = (
            "run_parameters.csv"
            if self.project_folder.project_folder is None
            else self.project_folder.output_file("run_parameters.csv")
        )
        with open(self.parameter_file, "w") as _:
            pass
        with open(self.parameter_file, "a") as write_file:
            ...
            if all_timesteps:
                with tqdm(
                    total=number_of_samples, position=0, leave=True
                ) as progress_bar:
    ...
    def _mask_parameters(self, mask_array, parameter):
        return xr.where(mask_array == 1, parameter, self.grid["grid_mask"])
    ...
    def calc_compaction_final(self):
        ...
        TwoD = len(self.pressures.shape) == 4
        nr_of_dimensions = max(
            [len(self[i].shape) for i in self.compaction_model.variables]
        )
        ...
        if not TwoD:
            if not hasattr(self.grid, "reservoir_mask"):
                raise Exception(
                    "Reservoir reservoir mask not set, run mask_reservoirs before calculating"
                )

            compaction = self._compaction_model.compute(
                self.grid.isel(time=[0, -1])
            ).astype(float)
