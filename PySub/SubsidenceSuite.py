...
class ModelSuite:
    ...
    def unique_observations(self, observations = None, model = None):
        ...
        """
        try: return self._unique_observations
        except:
            self._unique_observations = self.get_unique_observations(observations = observations, model = model)
    ...
    def get_unique_observations(self, observations = None, model = None):
        ...
        if observations is None:
            if self.hasattr('observation_points'):
                observations = np.unique([col.names for model_name, col in self.observation_points.items()])
            else:
                return
        else: 
            _observations = [col[observations] for model_name, col in self.observation_points.items()]
    ...
    def unique_relative(self, observation_name = None, model = None):
        ...
        if observation_name is None:
            if self.hasattr('observation_points'):
                observation_name = np.unique([col.names for model_name, col in self.observation_points.items()])
            else:
                return
        else:
            if type(observation_name) == list or type(observation_name) == np.ndarray:
                pass
            elif type(observation_name) == str:
                observation_name = [observation_name]
            else: 
                raise Exception(f'Data type {type(observation_name)} is invalid.')
        ...
        lower_error = []
        upper_error = []
        for m, (_, observation_points) in enumerate(self.observation_points.items()):
            if m in model_index:
                for i, name in enumerate(observation_points.names):
                    if name in observation_name:
                        integer_timestamps = observation_points.time[i].astype(int)
                        for j, observations in enumerate(observation_points.relative[i]):
                            names.append(name)
                            x.append(observation_points.x[i])
                            y.append(observation_points.y[i])
                            time.append(integer_timestamps[j])
                            observation.append(observation_points.relative[i][j])
                            lower_error.append(observation_points.lower_errors[i][j])
                            upper_error.append(observation_points.upper_errors[i][j])
        all_observations = np.array([names, x, y, time, observation, lower_error, upper_error])
        unique_observations = np.unique(all_observations, axis = 1)
        names, x, y, time, observation, lower_error, upper_error =  unique_observations
        time = pd.to_datetime([int(t) for t in time])
        points = []
        for i, name in enumerate(observation_name):
            indeces = np.where(names == name)
            p = _Points.ObservationPoint(name, x.astype(float)[indeces], y.astype(float)[indeces])
    ...
    def _check_dim1D(self, name, var, dims_equal = 'models', _set = True):
        try: var[0]
        except: raise Exception('Variable {name} must be array-like.')
        if dims_equal == 'models':
            if self.number_of_models == None:
                if _set: 
                    ...
                    raise Exception('Number of models has not been determined, set with appropriate variable or explicitly.')
            elif len(var) != self.number_of_models:
                self._dim_exception(name, dims_equal, self.number_of_models, self._variable_that_set_number_of_models)
    ...
    def set_raster_defaults(self, kwargs = {}):
        ...
        kwargs : dict
            ...
            https://matplotlib.org/stable/api/_as_gen/matplotlib.patches.Polygon.html
        """
        self._raster_defaults = _plot_utils.set_defaults(kwargs, defaults = self._raster_defaults)
        if self.hasattr('models'):
            for model in self.models:
    ...
    def get_contour_levels(self, variable = 'subsidence', levels = None, start = None, end = None, contour_steps = 0.01, drop_value = 0):
        ...
        if levels is None:
            if (start is not None and
                ...
                if self.hasattr('models'):
                    model_contour_levels = [m.get_contour_levels() for m in self]
                    min_values, max_values = list(zip(*[(np.min(c), np.max(c)) for c in model_contour_levels]))
                    min_value, max_value = np.min(min_values), np.max(max_values)
                    levels = _utils.stepped_space(min_value, max_value, contour_steps)
                    levels = _plot_utils.set_contour_levels(contour_levels = levels, contour_steps = contour_steps)
                    return levels
    ...
    def add_model(self, model):
        ...
        if _utils.isSubsidenceModel(model):
            ...
            raise Exception('Models part of ModelSuite object need to be a SubsidenceModel object.')
    ...
    def initiate_models(self, names):
        ...
        if not _utils.is_list_of_strings(names):
            raise Exception('Suite.inititate_models takes a list of strings representing model names.')
        ...
        if np.logical_not(valid_types).any():
            raise Exception('Model names need to be a list or 1D np.ndarray strings or integers')
        
        self.model_names = names
        
        for i in range(len(names)):
            model_project_folder = os.path.join(self.project_folder.project_folder, names[i])
            model = _SubsidenceModelGas.SubsidenceModel(names[i], project_folder = model_project_folder)
            ...
            if multi_dy is not None:
                ...
                for i, model in enumerate(self._models):
                    if hasattr(model, 'dx'):
                        ...
                        raise Exception('When setting dy for models, supply a dy. When dy is None, have previously set dx.')
    
    def set_influence_radius(self, multi_influence_radius):
        if self._check_for_models('set_influence_radius'):
            self._check_dim1D('dy', multi_influence_radius, dims_equal = 'models')
            for i, model in enumerate(self._models):
                model.set_influence_radius(multi_influence_radius[i])
            
    def set_compaction_model_parameters(self, multi_compaction_model, multi_compaction_coefficients, 
        ...
        if self._check_for_models('set_compaction_model'):
            ...
            self._check_dim1D('compaction_model', multi_compaction_model, dims_equal = 'models')
            ...
            self._check_dim1D('reference_stress_rates', multi_reference_stress_rates, dims_equal = 'models')
            ...
            for i, model in enumerate(self._models):
                model.set_compaction_model_parameters(multi_compaction_model[i], multi_tau[i], multi_reference_stress_rates[i], multi_density[i], multi_cmref[i], multi_b[i])
        
    def set_subsidence_model_parameters(self, multi_subsidence_model, knothe_angles = None,
                             depth_to_basements = None):
        if self._check_for_models('set_subsidence_model'):
            self._check_dim1D('subsidence_model', multi_subsidence_model, dims_equal = 'models')
            if knothe_angles is None:
                knothe_angles = [None] * self.number_of_models
            else:
                self._check_dim1D('knothe_angles', knothe_angles, dims_equal = 'models')
                
            if depth_to_basements is None:
                depth_to_basements = [None] * self.number_of_models
            else:
                self._check_dim1D('depth_to_basements', depth_to_basements, dims_equal = 'models')
                
            for i, model in enumerate(self._models):
                model.set_subsidence_model_parameters(multi_subsidence_model[i], knothe_angles = knothe_angles[i], depth_to_basements = depth_to_basements[i])
            
    def set_reservoirs(self, multi_reservoir_names):
        if self._check_for_models('set_reservoirs'):
            self._check_dim1D('reservoirs', multi_reservoir_names, dims_equal = 'models')
            for i, model in enumerate(self._models):
    ...
    def set_shapefiles(self, multi_shapefile_paths):
        if self._check_for_models('set_shapefiles'):
            ...
            self._check_dim1D('shapefiles', multi_shapefile_paths, dims_equal = 'models')
            for i, model in enumerate(self._models):
    ...
    def set_depth_to_basements(self, multi_depth_to_basements):
        if self._check_for_models('set_depth_to_basements'):
            self._check_dim1D('depths', multi_depth_to_basements, dims_equal = 'models')
            for i, model in enumerate(self._models):
    ...
    def set_compaction_coefficients(self, multi_compaction_coefficients):
        if self._check_for_models('set_compaction_coefficients'):
            self._check_dim1D('compaction_coefficients', multi_compaction_coefficients, dims_equal = 'models')
            for i, model in enumerate(self._models):
                model.set_compaction_coefficients(multi_compaction_coefficients[i])
    
    def set_thickness(self, multi_thickness):
        if self._check_for_models('set_thickness'):
            self._check_dim1D('thickness', multi_thickness, dims_equal = 'models')
            for i, model in enumerate(self._models):
                model.set_thickness(multi_thickness[i])
            
    def set_timesteps(self, multi_timesteps):
        if self._check_for_models('set_timesteps'):
            self._check_dim1D('timesteps', multi_timesteps, dims_equal = 'models')
            for i, model in enumerate(self._models):
            ...
            if self._check_for_models('set_bounds'):
                ...
                for model in self._models:
                    if model.hasattr('bounds'):
                        ...
                        raise Exception('Not enough information to base bounds on. Set bounds of this function, of the models in this Suite or set the shapes of the models in this Suite.')
                if len(shapes) > 0 and len(model_bounds) == 0:
                    ...
                    combined_bounds = np.array([model_bounds, np.arrray(shapes)])
                    bounds = np.zeros(4)
    ...
    def mask_from_grid(self, multi_grid):
        if self._check_for_models('mask_from_grid'):
            self._check_dim1D('grid', multi_grid, dims_equal = 'models', _set = False)
            for i, model in enumerate(self._models):
    ...
    def model_label_to_index(self, label):
        ...
        if self.hasattr('models'):
            if label is None:
                ...
                raise Exception(f'Invalid model label type: {type(label)}. Enter integer or string or list of integers or strings.')
    ...
    def calculate_subsidence_at_observations(self):
        ...
        if self.hasattr('models'):
            ...
            result = {}
            for model in tqdm(self._models):
                value = model.calculate_subsidence_at_observations(_print = False)
                result[model.name] = value
    ...
    # Get from results
    def get_timeseries(self, x, y, variable = 'subsidence', reservoir = None, model = None):
        ...
        if self.hasattr('models'):
            ...
            for i, m in enumerate(self._models):
                if i in model:
                    timeseries[m.name] = m.get_timeseries(x = x, y = y, variable = variable, reservoir = reservoir)
            return timeseries
    ...
    def get_max_subsidence_timeseries(self, reservoir = None, model = None):
        if self.hasattr('models'):
            ...
            for i, m in enumerate(self._models):
                if i in model:
                    timeseries[m.name] = m.get_max_subsidence_timeseries(reservoir = reservoir)
            return timeseries
    ...
    def get_max_subsidence(self, time = None, reservoir = None):
        ...
        if self.hasattr('models'):
            ...
            which_model = []
            for m in self._models:
                s, (x, y) = m.get_max_subsidence(time = time, reservoir = reservoir)
                smax.append(s)
                xs.append(x)
                ys.append(y)
    ...
    def get_min_subsidence(self, time = None, reservoir = None):
        if self.hasattr('models'):
            smin, xs, ys = [], [], []
            for m in self._models:
                s, (x, y) = m.get_min_subsidence(time = time, reservoir = reservoir)
                smin.append(s)
                xs.append(x)
                ys.append(y)
            imin = np.argmax(smin)
